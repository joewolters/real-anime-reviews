...
- **Project rules** — the 7 binding rules (catalog-canonical, mark-changes, modes, test-before-commit…).
...
1. **The catalog is canonical.** <!-- author: Code | date: 2026-08-09 --> `catalog/{animeId}` in Firestore is the source of truth for anime data. `animeData.js` is GENERATED from it by `npm run catalog:publish` — never hand-edited. Edit through `/admin/catalog` (works from any device); every write keeps an append-only revision with one-click undo, and publish REFUSES a material shrink in review text. Excel is now an **export** (`npm run catalog:xlsx`), not an input — `Master List/Anime_Master_Table.xlsx` is archived, not deleted, and `npm run sync` refuses to overwrite from it without an explicit override flag. *(Changed in v2.0.1's cloud migration. Excel was canonical from v1.5.0 until 2026-08-09; the May 2026 silent revert of four reviews is why it isn't any more — see `docs/CLOUD-MIGRATION-STUDY.md`.)*
