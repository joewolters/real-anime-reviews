...
**Up next:** v1.7.2 — multi-fetch data architecture + multi-hop revival (closes the v1.6.10 architectural debt — multi-hop franchise traversal + franchise-episode aggregation both land via batched parallel fetches, now unblocked by v1.7.0's id backfill). After that: v1.7.3 — in-site secondary modal (replaces v1.6.8's open-AniList-in-new-tab with an in-site detail view, requires v1.7.2's data layer); v1.8.0 — AniList tab on cards (at-a-glance verified-source data).
...
### v1.7.2 — MINOR — Multi-fetch data architecture + multi-hop revival
...
### v1.7.3 — MINOR — In-site secondary modal for franchise entries
...
### v1.7.x polish slots (post-v1.7.3)
