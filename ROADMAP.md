...
| **v1.8.4** (MINOR) ← **next** | **Discovery & Blend** — three adjacent surfaces (Blake's Reviews / For You / Discover), one card shell + `reviewed` flag, NOT-REVIEWED via the existing secondary modal; + the v1.8.3 postponements (characters on the page, the designed background-reveal concept) and a Quotes-admin page (manage welcome quotes + ✨ASK drawer). Own gate-0 design study; seeds in CODE-HANDOFF/NEXT.md |
| **v1.9.0** (MINOR→MAJOR) | Community/Account/User overhaul (comments incl. per-season, community reviews, notifications, DM inbox, account redesign) + Cloud Functions (notification pruning, cascade deletes) + privacy notice |
