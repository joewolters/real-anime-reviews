...
**Up next:** v1.7.0-polish — widget version chips per update-log `<li>` (small visitor polish: each bullet carries its source-version chip per the v1.6.4 convention; ~1-2h). After that: v1.7.1 — multi-fetch data architecture + multi-hop revival (closes the v1.6.10 architectural debt — multi-hop franchise traversal + franchise-episode aggregation both land via batched parallel fetches, now unblocked by v1.7.0's id backfill); v1.7.2 — in-site secondary modal (replaces v1.6.8's open-AniList-in-new-tab with an in-site detail view, requires v1.7.1's data layer).
