...
**Up next:** v1.6.10 — Multi-hop franchise polish + per-entry studio dedupe. Extend franchise traversal (both `aggregateFranchise()` in `admin/new-anime.js` AND the public modal's relations fetch in `script.js`) to recurse one hop past direct relations with a depth cap of 2 — closes the OPM-S3-missing limitation; per-entry studio dedupe one-liner — closes the MADHOUSE,MADHOUSE limitation. Plus two v1.6.9-smoke additions: a format badge on the franchise relation rows (distinguish Movie/TV/OVA entries), and franchise-episode aggregation (fetch + merge each season's `streamingEpisodes`). After v1.6.10: v1.6.11 — Suggestion Box + admin viewer integration; v1.6.x — real one-click AI integration via Firebase Cloud Function (see `docs/ai-integration-design.md`).
...
- **Watchlist + "Not Reviewed yet" on ALSO LIKED.** When a visitor clicks an ALSO LIKED card (a recommended anime not in Blake's catalog), instead of opening AniList in a new tab the secondary modal opens IN the site showing "Not yet reviewed" state + an "Add to watchlist" CTA. Watchlist functionality (storage, UI, per-user persistence) is a prerequisite — may need a separate v1.7.x or v1.8.x ship for the watchlist itself.
