...
**Up next:** v1.7.0 — AniList backfill + MAL integration. Foundation ship for the v1.7.x arc. Populates `AniListId` / `IdMal` / `AniListScore` / `AniListColor` columns on the existing ~44 reviews via Mode 1's pipeline. Excel + `animeData.js` both updated. Once shipped, every modal fetch can use the precise `Media(id:)` lookup instead of the popularity-sorted `Page(media:)` search — prerequisite for v1.7.1's multi-fetch architecture. After v1.7.0: v1.7.1 — multi-fetch data architecture + multi-hop revival (closes the v1.6.10 architectural debt — multi-hop franchise traversal + franchise-episode aggregation both land here via batched parallel fetches); v1.7.2 — in-site secondary modal (replaces v1.6.8's open-AniList-in-new-tab with an in-site detail view, requires v1.7.1's data layer).
