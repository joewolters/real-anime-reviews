...
**Up next:** v1.7.3 — Watched-set feature (multi-season `✓ Reviewed` via a new `WatchedAniListIds` column + admin multi-select reusing v1.7.2's `traverseFranchise` + Mode 1 auto-fill + a ~20-min backfill across the 44; slotted in 2026-06-03 during v1.7.2 gate 4). After that: v1.7.4 — in-site secondary modal (replaces the open-externally jump; benefits from v1.7.3 because pills will work correctly across all watched seasons); v1.8.0 — AniList tab on cards (at-a-glance verified-source data).
