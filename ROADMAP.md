...
## ⚡ READ-FIRST
- **Live at v2.0.0 — THE MEGA-RUN MILESTONE** (deployed 2026-07-04 on Blake's go; full runbook order incl. the profiles backfill he ran himself, minted 7 + existing 5 = 12 ✓): the messaging era (peer letters request-first, groups, sealed images, the Letter Room), the ONE Discord-grade composer everywhere, rarNav (every view shareable + honest Back), the Curator Studio + card stickers + the den line, Hidden Gems + the yellow tape, member shelves with the on-card featured pick, the Constellation, emails private, the responsive rebuild 320px→ultrawide, the de-Blake voice sweep, the og door image. Post-cutover queue: `docs/NEXT.md` 💭 CURRENT (one carry-over stands: review-targeted notification deep-links don't always scroll-to-highlight).
...
**v2.3.5 — THE SHARE BUTTON, STRAIGHTENED; AND SEASONS CAN SHARE TOO.** Blake, on the v2.3.4 button: *"looks uneven. Make sure it looks good on mobile too. Also make sure this is possible with seasonal reviews."* All three were real. **(1)** It was `display:inline-flex`, and an inline box only centres if its PARENT is `text-align:center` — this modal centres each child individually (`.modal-title` style.css:1381, `.modal-romaji` 7373), so the button sat hard left under a centred title. Now `display:flex; width:fit-content; margin:0 auto`. ⚠️ The spec asserts button-centre vs title-centre within 2px at BOTH 1280 and 360 — a `toContain('display:flex')` check would have passed against the broken version. **(2)** Mobile: it was ~28px tall against the ~44px Apple/Material both want; now `min-height:44px` plus `max-width:100%`, with the no-sideways-scroll assertion pinned at 360. **(3)** Seasons: the secondary layer is keyed by AniList id, so `/season/<id>` is a second rewrite onto the SAME `animePreview`. A season review stores only id/title/rating + prose in `content/body` and **no cover**, and a crawler cannot follow an `onerror`, so the function fetches AniList art server-side and overlays Blake's own fields; AniList synopses are HTML, so the blurb builder now strips tags and entities or a literal `<br>` renders in the card. The pill matches `.secondary-edit-review` (38px, collapses to an icon ≤900px) but in neutral purple — NOT the Edit button's gold, which is Blake's own voice. ⛔ **THE BUG THIS UNCOVERED:** a phone-width test failed with no confirmation ever appearing — the click arrived, the handler ran, and nothing happened. `navigator.clipboard.writeText` **does not always settle**: with the document unfocused some browsers leave the promise PENDING FOREVER rather than rejecting, so `await`ing it never reaches the confirmation. An earlier browser check had missed it precisely because clipboard was *denied* there, which rejects fast. Now `copyToClipboard()` races a 1200ms timeout then falls back to `execCommand('copy')` and always returns a boolean; feedback is shown IMMEDIATELY and corrected after, never awaited (a button inert for a second reads as broken — the same complaint that started this); and `_shareTimer`/`_shareWas` on the element stop a second click stranding a half-reverted label. Floors: `npm test` **435 → 445** · rules 222 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-30 -->
**v2.3.4 — PUBLISHING IS ONE STEP, AND A SHARED REVIEW SHOWS THE REVIEW.** Blake: *"Can we fix both those issues"* (publishing takes two steps; the publish script never checks cover art) and *"I posted a link with a review. Can the thumbnail of the review show up instead of the entire page?"* **(1) THE LIVE TOP-UP.** The site loads its generated `animeData.js` exactly as before — study §3's static-publish decision STANDS (zero per-visitor reads, no async before first paint, ~20 catalog-coupled specs untouched, and "a boot-path rewrite on a live site with real members" was its HIGH risk). After first paint it asks one question: `where('updatedAt','>', RAR_CATALOG_PUBLISHED_AT)` — empty on a normal visit, non-empty only between a publish and the next rebuild. Not awaited, swallows its own errors, so a failure leaves today's behaviour exactly. Rules needed nothing (`firestore.rules:569` is already `allow get, list: if true`). The generated file now ships its own schema (`RAR_CATALOG_PUBLISHED_AT` + `RAR_CATALOG_FIELDS`, from the same ALWAYS/OPTIONAL the renderer emits), both before the `const animeData = [` marker so the byte-exact round-trip is untouched. ⛔ **THE TRAP THAT ALMOST SHIPPED:** the obvious design — put the extended catalog on `window.animeData` — is silently half-broken. `animeData` is a top-level `const` in a CLASSIC script, a global LEXICAL binding, NOT `window.animeData` (undefined on index.html); and script.js has 8 dual-source read sites with INCONSISTENT precedence (five window-first, three const-first including `openAnimeFromId`) plus ~25 bare-const sites, so a second array means a SPLIT-BRAIN catalog — grid at 45, deep-link router at 46. The binding is const but the ARRAY is not: `catalogTopUp` mutates the one array in place and invalidates the three memoized maps (`_catalogBySlug`/`_primaryIdToSlug`/`_watchedIds`), then re-runs only the catalog-derived renders (each checked to bind no listeners, so idempotent). **(2) COVER ART FAILS LOUDLY.** A cover tripwire in `catalog-publish` (shrink-tripwire voice, same `--force`): missing WITH an `AniListCover` warns, missing WITHOUT one refuses. `AniListCover` is stored at publish and appended LAST to OPTIONAL so no existing entry's bytes move; cards fall back local → AniList → placeholder via `data-fallback` + a loop-proof `data-fb` latch — which matters *because of* (1), since a review is live seconds after publishing while its art is not. The Add Anime page now probes the file instead of asserting it. **(3) LINK PREVIEWS.** Not a bad tag: `#anime=` is a FRAGMENT and a fragment is NEVER sent to a server, so Discord asked for `/` and correctly got the homepage card. New `/anime/<slug>` hosting rewrite → `animePreview` (onRequest, reads the catalog doc live so a fresh publish previews at once) → per-anime og/twitter tags, then bounces humans into the app; crawlers keep the tags, `<noscript>` covers the rest, and an unknown slug returns the site card at 200 rather than a 404. Card type `summary`, not `summary_large_image` — a cover is 2:3 portrait and the large card letterbox-crops it. A **Copy share link** button on the modal is what actually delivers this, since the address bar still shows the un-previewable `#anime=` form. ⚠️ Two test traps recorded in the spec: the deep-link route runs at LOAD ONLY (no `hashchange` listener), and `page.goto('/#x')` from `/` is a same-document navigation that never reloads. ❗ **Deploy order is not interchangeable: functions FIRST, then hosting.** Floors: `npm test` **424 → 435** · rules 222 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-29 -->
**v2.3.3 — THE FIRST NEW REVIEW SINCE THE CLOUD MOVE, AND "LATEST DROP" NOW MEANS LATEST.** Blake: *"okay well I posted it but I don't see it on the website? and when it does I need it to show up on latest drop."* Two real things. **(1)** His publish DID save — the v2.3.2 fix held, catalog 44 → 45. The site does not read Firestore live (study §3: static publish, not live reads), so a saved review is invisible until `catalog:publish` regenerates `animeData.js` and it deploys. Done, with the AniList cover (id 169580) fetched to `assets/i-made-friends-with-the-second-prettiest-girl-in-my-class.png` at 400×600 PNG to match the other 44. **(2)** ⚠️ **The publish alone would NOT have shown it to him.** `pickFeaturedAnime()` (v1.8.4 gate 4) personalized the slot for signed-in members — most-recent favorite → watchlist → recent history → *then* the newest review — while the widget is labelled "LATEST DROP 最新 / Now Featuring". So the label described something the slot was not doing, and a brand-new review could go up and never appear in the one panel that promises new things. Blake's screenshot showed Black Clover: his most-recent favorite. **Asked rather than assumed**; he chose "always the newest, for everyone", so the pick is now unconditionally the catalog tail (`animeData` is emitted sorted by the `order` field and a new anime takes the highest, so the tail IS the newest). FOR YOU and Continue Where You Left Off still do the personalizing. ⚠️ **A TEST TRAP RECORDED:** the obvious "mutate `window.favoritesSet` and re-pick" test proves NOTHING — those sets are module-scoped (`script.js:229`) and never exported, so it passes against the old code too, the same green-over-nothing that hid the dead Publish button one version earlier. `tests/v233-latest-drop-is-latest.spec.js` reads the LIVE function via `Function.prototype.toString()` and asserts the body never names the save sets; verified to fail on the pre-change body. Also: `admin/catalog.html`'s one-time seed said "Import 44 anime" — it counted, so it went stale at 45; it no longer names a number. Floors: `npm test` **420 → 424** · rules 222 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-29 -->
**v2.3.2 — THE PUBLISH BUTTON WAS DEAD.** Blake: *"when I go to publish a new review nothing happens."* `admin/new-anime.js` still called `validateBeforeGenerate()` — a function **v2.3.0 deleted with the Mode 1 Excel pipeline, leaving the call behind.** It is the first statement in the Publish click handler, so every press threw `ReferenceError: validateBeforeGenerate is not defined` before `publishToCatalog()` ever ran: no write, no error text, no spinner. ⚠️ **The v2.3.0 test was green over it** — it reads the button's LABEL and asserts the page boots clean, and never clicks; a dead button passes all of that at rest. The check is restored but thinner: the overlapping rules defer to `RarCatalogModel.validate` (one source of truth with the sync and the Cloud editor) and only the AniList fetch, `Seasons`, `Studio` and the image override stay local. `collectCoreFields()` makes the pre-flight check and the write ONE reader of the form, so a field cannot be validated in one shape and saved in another. The kicker on that page said `MODE 1 モード1` — the same stale label that misled him last time — and now says `ADD ANIME 追加`. `tests/v232-publish-button-alive.spec.js` guards the CALL GRAPH (every helper the Publish path names must be declared) and was verified to fail against the pre-fix file; the same parser swept every `.js` on the site and found no other dead call. A read-only `catalog-publish --from=rest` confirms the shipped `animeData.js` is **identical to the cloud catalog** (44 entries). Floors: `npm test` **417 → 420** · rules 222 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-29 -->
**v2.3.1 — NOTHING ON THE ADMIN SIDE RUNS ON THE OLD DESKTOP SERVER ANY MORE.** Blake: *"make sure season reviews or edits of any kind don't operate on that old hardware anymore."* Three pages still did: `admin/edit` (`PUT /api/anime/:slug`), `admin/quotes` (`PUT /api/quotes`) and `admin/season-reviews` (`/api/season-review` CRUD). ⚠️ **Season reviews had NEVER worked** — `index.json` shipped `count: 0` because writing one was impossible without that machine. New homes: catalog edits → `catalog/{animeId}`; quotes → `siteContent/quotes`; season reviews → `seasonReviews/{id}` (light head: id/title/rating) + `content/body` (the prose), so the site's LIST read stays cheap however long reviews get and **there is no index file to regenerate or drift**. ⚠️ **THE TRAP CAUGHT BEFORE IT SHIPPED: `edit.js` has its own `slugify()` that STRIPS apostrophes** (it had to match the old server's row lookup) while catalog ids turn them into a dash — `an-archdemons-...` vs `an-archdemon-s-...`, a silent miss on 8 titles, and that id is the key every live comment room hangs off. The id now comes from the shared `RarCatalogModel.slug`, and the write is `updateDoc` (never set-with-merge, which would mint a ghost row). `scripts/lib/platform-map.js` **MOVED** to `admin/platform-map.js` and became dual-mode so the "fix platforms" helper runs in the browser on the SAME rules as the backfill CLI — moved rather than copied, because two copies of an allowlist drift. The Edit page's Tier-2 "Ship live" (a git commit + deploy streamed from the server) is **removed**: publishing is a deliberate step from the machine, as it is for Add Anime. The door reads quotes cloud → shipped file → seeded list, so it can never end up empty. **New rules:** `siteContent` (public read, admin write, no delete, no list) and `seasonReviews` (+ `content` child; public read/list, admin write, `id` must be an int). Floors: `npm test` **417** · rules **218 → 222** · webkit 24.

<!-- author: Code | date: 2026-08-13 -->
**v2.3.0 — ADDING AN ANIME WORKS AGAIN (the cloud publish path).** Blake, holding a finished review he could not post: *"the website posting anime admin section thinks we need mode 1 to publish the excel role... We have moved onto the cloud."* ⚠️ **He was right, and it was worse than a stale banner: `admin/new-anime` could only ship via the Mode 1 desktop server or an Excel row, and the cloud migration retired BOTH — so adding a new anime had been IMPOSSIBLE since v2.0.1 and nobody had tried until now.** Fixed with `publishToCatalog()`: one `setDoc` to `catalog/{animeId}`, from any device, no desktop server. ⚠️ **The slug comes from the SHARED `RarCatalogModel.slug`, never a local copy** — it is the doc id AND the key every live comment room hangs off; `firestore.rules` already enforced `animeId == slug == docId`, so **no rules change was needed and this is hosting-only**. The page had to gain `catalog-model.js` (it was not loaded there — the publish would have failed instantly; caught by driving the page, not by reading it). **DELETED, not commented out:** the Excel row builder, the commands block, the Mode 1 Submit-and-Ship pipeline with its SSE progress UI, the server probe, and every `serverMode` branch. Covers stay FILENAMES — ten separate places build `assets/<file>`, so the art is fetched at rebuild time instead of touching all ten. ⏳ **STILL ON THE OLD SERVER, and Blake has asked for them next: `admin/edit` (superseded by `/admin/catalog`), `admin/season-reviews` (⚠️ count is ZERO — nobody has ever been able to write one) and `admin/quotes`.** Those write FILES the site serves, so they need a store the site can read — a rules change, unlike this one. Floors: `npm test` **410** · webkit 24.

<!-- author: Code | date: 2026-08-13 -->
**v2.2.5 — the Den as the front door, the account page on phones, and a sign-in diagnostic.** Blake: *"The website should open to the Den. Not my anime cards"* · *"account page looks INCREDIBLY squished together and cluttered. Everything needs to be even and look spaced correctly like the admin tiles"* · *"Still can't log in in safari/duckduckgo"*. **Routing:** a bare `#all` is a TOOL view (rarNav lights no nav place for it) and is now normalised away on a COLD load, so a URL-restoring browser lands in the Den; every content route is untouched. ⚠️ **This broke `g32-responsive`'s four `gD` cases, which used `/#all` as a shortcut to the grid — a REAL catch, not a flake.** They now click through as a person would (and open the nav drawer first below 1200px, which is why two of the four still failed on the first rewrite). **Account page:** `.account-side nav` was `display:flex; overflow-x:auto` — a scroller inside a 350px card that clipped tab labels mid-word ("Favor"). It is now an even 2-column TILE GRID with admin-tile chrome, a 44px tap floor, greeting and Sign-out on one row, card padding 26→14px, even 6/4-column colour grids, and 72px of bottom clearance for the admin FAB. **Diagnostic:** `?authcheck=1` prints the session store, whether site data is writable, IndexedDB availability and the live auth state — opt-in, invisible otherwise, and it polls until the tier resolves so it never shows "pending". ⚠️ **Safari/DuckDuckGo sign-in is STILL UNSOLVED** — two hypotheses have now failed, and the diagnostic exists so the third attempt starts from facts. Floors: `npm test` **407** · webkit 24.

<!-- author: Code | date: 2026-08-12 -->
**v2.2.4 — THE BARE-SPECIFIER INCIDENT + the cache-header fix** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-08-12). ⚠️ **A SELF-INFLICTED OUTAGE, owned:** v2.2.3 added `authPersistenceReady` to script.js's import list from `'./firebase.js'` — a **BARE specifier with no `?v=`**, which is a SEPARATE cache entry from the versioned copy the page loads. Any browser holding a stale bare copy threw on the missing export and **script.js never executed**: the static shell rendered while nothing dynamic loaded. Blake: *"it has the admin thing in the bottom left but nothing is loading and I can't access my account"* — `admin-fab.js` imports the same bare module but needs only `auth`+`db`, which the old copy still exports, so the FAB survived alone. The import is withdrawn (the tier is read from `window.__rarAuthPersistence` instead, which cannot break a stale client), the bare-imported modules now serve `no-cache`, and **two guards pin it**: script.js's firebase import list is asserted to be exactly the four long-standing names, and the no-cache rule must sort AFTER the generic asset rule or it would be overridden. **Also fixed:** there was NO `headers` config at all, so Firebase's default `max-age=3600` meant every release was invisible for up to an hour — and a stale page requests the previous release's `?v=` assets, defeating the cache-busting via the one file it depends on. Floors: `npm test` **404** · webkit 24.

<!-- author: Code | date: 2026-08-12 -->
**v2.2.3 — THE SESSION-PERSISTENCE FIX** (built after Blake reported he *still* could not sign in on his phone, in TWO browsers: Arc said "password and email are wrong"; DuckDuckGo "brings me to the home page and actually doesn't sign me in"). ⚠️ **The DuckDuckGo half is the diagnostic one — the credential is ACCEPTED and the session then isn't there.** `getAuth()` persists to **IndexedDB** by default, which is precisely what privacy-first browsers block/partition/wipe; when it fails the SDK can fall back to **in-memory**, where the session dies on the next navigation and is indistinguishable from "it didn't sign me in". Persistence is now chosen EXPLICITLY, strongest-first (`browserLocal` → `browserSession` → `inMemory`), settled BEFORE the credential call, and the winning tier is published on `window.__rarAuthPersistence`. If only a non-durable tier is available the sign-in form **says so and keeps the modal open** rather than closing on a false success. The "incorrect email or password" copy now points at the email box, because the usual phone cause is autofill supplying a different saved address. ⚠️ **HONEST STATUS: this is a strong, evidenced hypothesis for the DuckDuckGo symptom and a diagnostic improvement for Arc — it is NOT a reproduction.** Neither browser's failure could be reproduced locally; WebKit at three iPhone sizes signs in fine. Floors: `npm test` **401** · webkit 24.

<!-- author: Code | date: 2026-08-12 -->
**LIVE at v2.2.2 — the sign-in silence, the shelf picker, and the rest of the phone sizing** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-08-12 on Blake's "deploy it with proper update notification"; hosting only). 🐛 **The sign-in silence was OURS and was owned as such:** v2.2.1 made the visible box the autofill target but left it `type="email"`, so a stored username failed browser validation and the form **never submitted at all** — no request, no error. Now `type="text"` + `inputmode="email"`, validated in our own handler, with **every exit path speaking** and the message scrolled into view (it sits at the bottom of a panel that scrolls on a phone). 🗂️ **BANKED ITEM 6 SHIPPED:** the shelf picker leads with anime they have REVIEWED, then their lists, then the catalog, as labelled groups — built as a **source registry** so the future AniList sync is one array entry rather than a second picker; a reviewed catalog title is promoted (not duplicated) and borrows its cover; the reviewed read never blocks first paint. 📐 **The rest of Blake's 8-item list:** genre rails were 246px wide starting at x=72 inside a framed panel — now **0→390, edge to edge**; Latest Drop 84% of the screen → a cover beside its text; the Top-10 shrank **on his own reversal** and now scales with the screen (a fixed block is 47% of an iPhone 15 but 70% of an SE); the after-Enter results and Continue Where You Left Off were falling through to full-width in a wrap grid. 🛡️ The CUTOVER-EVE spec then caught the tallest Top-10 card spilling 4px past its frame — exactly the bug it exists for — and two tests that pinned the reversed 275px number were rewritten to guard what they were actually for. Floors: `npm test` **396** · rules 218 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-12 -->
**LIVE at v2.2.1 — the iPhone sign-in fix + the compact pass everywhere** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-08-12 on Blake's *"deploy it and push it with proper update notification etc"*; hosting only). 🐛 **The sign-in bug was real and it was ours:** the auth form advertised `autocomplete="username"` on a **hidden, still-enabled** input while the visible identifier said `email`, so iOS / iCloud Keychain filled the invisible box and the visible Email field submitted empty → Firebase `user-not-found` → *"Incorrect email or password"* on phone only, with credentials that worked on desktop. The email now carries `autocomplete="username"`, the display-name field is `nickname` and **disabled whenever hidden** (disabled inputs are excluded from autofill AND submission), `openAuth()` re-arms per mode, and the email box is hardened against iOS autocorrect. ⚠️ Layout was **proven innocent first** at 393×852 and 393×516 in WebKit — and that negative was reported as a negative, since Playwright cannot reproduce the iOS keyboard. 🔁 **The compact pass was RESTRUCTURED, not extended:** v2.2.0 named four containers, so the genre rails (**220×616 = 73% of the screen**) and the search grid (174×526 = 62%) were left at desktop sizing. `.card .info` + `.card-corner` are now styled site-wide on phones with `.spotlight-stack` as the ONE explicit opt-out; the catalog/search grid went 2→3 columns. Measured 320-430: genre **117×310**, search **114×305**, genre and home rails within 2px. 🛡️ The spotlight opt-out is load-bearing and **mutation-tested**. The Tavern gets a trim (padding/chrome down, body text ≥13px). Floors: `npm test` **392** · rules 218 · cf 94 · functions 94 · webkit 24.

<!-- author: Code | date: 2026-08-12 -->
**LIVE at v2.2.0 — THE ENTER-PAGE OVERHAUL** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-08-12 on Blake's explicit "deploy it. make sure to update the changelog and whatnot"; HTML/JS/CSS only → `--only hosting`, no rules and no functions touched). The welcome door's catch-up strip is **deleted** on both phone and desktop; pressing **Enter** now opens the "while you were away" surface, rebuilt as two panels (airing from your watchlist | your lantern) under a full-width new-reviews rail, every row deep-linking to the exact anime or the exact comment. A quiet visit skips it entirely. ⚠️ **A gap the handoff had asserted was already closed, and was not:** airing rows called `openSecondaryFromKey` unconditionally, so a title Blake HAD reviewed opened the AniList deep-dive instead of his review — the branch he asked for by name is now `openAiringTarget()`. Enter also awaits the async signals under a bounded cap (settle in a `finally`, because the path below it has early returns) so a fast press can no longer silently skip the surface for a member with unread letters. **Mobile:** the home-rail cards had **no phone override at all** — a fixed 200×581 at every width, 69% of an 844px screen for one card, 1.79 visible — now 117×307 / 3.06 visible / 36%, with the title clamped (narrowing alone had moved the height by 16px; the text was the real cause). 🛡️ `.spotlight-stack .card{width:275px}` deliberately untouched **and now test-guarded at four widths**. **Search:** the header box now shows a `NOT REVIEWED YET` shelf beneath the reviews, reusing Discover's own search + ranking split + card, rendered BEFORE the no-match early return (the un-reviewed search is exactly when the 44 come back empty) and debounced 420ms for the AniList 30/min quota. **Admin:** the menu is a scrolling tile grid, centred on desktop, all tiles one size — it previously had no `max-height` and no `overflow` and ran off the top of the screen at eleven tools. Floors: `npm test` **387** · rules 218 · cf 94 · functions 94 · webkit 24. Record: `docs/SHIP-OUTPUT.md` + the newest `docs/v1.10.0-GATE-LOG.md` entry.

<!-- author: Code | date: 2026-08-10 -->
**LIVE at v2.1.0 — PART A COMPLETE** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-08-10 on Blake's explicit "deploy it, rules then functions then hosting"). **The deploy order deviated from the default runbook ON HIS WORD and for a stated reason:** hosting-first exists to stop a count-writing client coexisting with a count CF, which this ship does not touch — going rules → functions → hosting instead closed the console deletion hole FIRST and meant no member ever met a "Delete Account" button whose backend wasn't live. Verified live rather than assumed: `APP_VERSION` 2.1.0 + the widget chip agreeing, `admin/stats` and `account` resolving 200, an unauthenticated read of `adminStats/current` returning **403** while the public catalog still returns **200**, and `docs/**`, `tmp*` and `PERSONAL.md` all 404. All 43 functions live including the three new ones (`deleteMyAccount`, `recomputeStats`, `refreshStatsNow`). ✅ **Blake's human smoke PASSED 2026-08-10** — including the member-stats first Refresh, so `refreshStatsNow`'s collection-group read needed **no extra index** (the one live unknown at ship). Part A is closed end to end. Self-serve account deletion under the honest policy "tombstone the containers, erase the content" — and the cascade that used to destroy OTHER members' posts when someone left is gone with it. Two live holes closed on the way: `users/{uid}` was owner-deletable and detonated the whole account cascade from a browser console, and the redaction path could resurrect just-deleted documents as entries nobody wrote. Plus the profile reviews rework (a pinned review + one "all reviews" door, and the one-tab tablist removed as the accessibility lie it was), the admin member-stats page (one scheduled recompute, one admin-only document, one read per page open), the iPhone/WebKit round (the account button was off-screen under ~390px, so sign-in was impossible), the legacy-member profile fix, the branded-select sweep, and the header search back in proportion. Floors: `npm test` 350 · rules 218 · cf 94 · functions 94 · webkit 24. **Deploy order for this ship deviates deliberately from the default runbook** — firestore rules → functions → hosting, so the deletion hole closes first and no member meets a button whose backend isn't live yet (the default hosting-first order exists to protect the vote-count model, which this ship does not touch). Record: `docs/PART-A-REMAINING.md` + the newest `docs/v1.10.0-GATE-LOG.md` entry.

<!-- author: Code | date: 2026-07-04 -->
**Live at v2.0.0 — THE MEGA-RUN MILESTONE** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-07-04 on Blake's explicit "ship it" — the full runbook order: backfillProfiles alone → Blake's own console backfill (minted 7 + existing 5 = 12, verified) → indexes → hosting → firestore rules → storage rules → functions; prod curl-verified APP_VERSION=2.0.0; cold shared-link + Back-walk + Top-10 phone pixels smoked live) — the messaging era (request-first peer letters, groups to 15, images sealed until accept, the Letter Room), the one live composer everywhere, rarNav shareable views + honest Back, the Curator Studio + animeStatus card stickers + the den watching line, Hidden Gems + the yellow-tape unreviewed modals + community rooms on everything, member shelves with the featured pick on the card, the Constellation, the users-GET privacy tightening behind the profiles backfill, the 320px→ultrawide responsive rebuild, the de-Blake sweep, the og door preview (full record: `CHANGELOG.md` v2.0.0 + `docs/v1.10.0-GATE-LOG.md`). **Live at v1.10.2** ([realanimereviews.com](https://realanimereviews.com), deployed 2026-07-02 on Blake's explicit go — hosting + firestore rules; prod curl-verified APP_VERSION=1.10.2) — the Creator profile per Blake's clarified spec (a member sheet, his own Studio choices, the gold CREATOR kicker the one difference, Appreciate ON by his word), the full account nav (View All · Random · Filter), the member-reported prod bugs dead at the root (pinned-review picker crush, /admin/admin 404s, the suggestion-queue detail + clipping), the admin Mode-1 honesty banners, and the Mode-1 desktop launcher working like an app. **Live at v1.10.1** — the v1.10.0 **Community Hub overhaul** + the upload/friendly-errors hotfix, shipped 2026-06-11/12 (the Tavern forum, moderation spine + community rules + reports/bans, the image pipeline on locked-down Storage, public member profiles with frames/accents/backgrounds, DM-Blake messaging, rooms on every season, the catch-up lantern system; deployed in the gate-21 cutover: indexes → hosting → firestore rules → storage rules → functions; full record in `CHANGELOG.md` + `docs/v1.10.0-GATE-LOG.md`). Prior history below. Foundation complete; Phase A shipped; Mode 1 baseline + server shipped (v1.6.0); spawn-EINVAL hotfix shipped (v1.6.1); Bug 10 prevention ship — startup smoke check + DECISIONS lesson — shipped (v1.6.2); polish bundle + first widget update under the new visitor-first skill shipped (v1.6.3); update log widget upgrade — dates, grouping, 10-cap, scroll containment — shipped (v1.6.4); Mode 1 live preview as you type (search-as-you-type + ID-import + card-render extraction) shipped (v1.6.5); cover-image fill hotfix (`object-fit: cover` so AniList covers no longer letterbox) shipped (v1.6.6); admin form franchise aggregation Part A (FRANCHISE INFO panel + franchise-aware Seasons/Studio prefill + PREQUEL warning, single-hop scope) shipped (v1.6.7); More Info panel Part B+ — visitor-facing franchise relations on the public anime modal, every season a card with cover/title/meta/score and a click-through to AniList — shipped (v1.6.8); Richer Modal Data — per-episode names, AniList recommendations, and key staff credits added inline to the More Info panel for the source anime — shipped (v1.6.9); More Info panel polish bundle — per-row studio dedupe, small format pill on each franchise row, STAFF cluster cap bumped 4→6 — shipped (v1.6.10); Suggestion Box + admin viewer — public `/suggest` page with live search-as-you-type + selection card + premium UI throughout, admin queue at `/admin/suggestions` accessible from the floating Admin pill with Add/Mark-reviewed/Delete actions + AniList-payload handoff to Mode 1 — shipped (v1.6.11); admin Suggestions Queue patch — error-card-on-success fix, custom branded delete-confirmation modal replacing native `confirm()`, and a side-by-side NEW/REVIEWED column split with `[hidden]` cascade-symmetry fixes — shipped (v1.6.12); AniList enrichment — one-time backfill of 6 fields on the legacy 44 reviews (40 matched / 4 skipped) via the new `npm run backfill` CLI, a community-score twin badge (`RATING · X` / `ANILIST · Y`) on every anime modal, and precise `Media(id:)` franchise lookups activating the v1.6.8 id-path — shipped (v1.7.0); AniList enrichment polish bundle — Japanese/romaji subtitles on cards + modal (`pickSubtitle` + native fallback, 3-line wrap), per-anime `AniListColor` badge accent with luminance guard, premium `NO MATCHES` empty-state, update-log version chips, the 4 skipped anime backfilled (all 44 enriched), and the `--add-native` / `TitleNative` pipeline — shipped (v1.7.1); More Info panel overhaul — multi-fetch data architecture (batched parallel fetches + BFS multi-hop franchise traversal) + per-season episode aggregation with byte-identical-list dedupe, a panel UX redesign (spine chain + connector line + relationType-grouped sections + `✓ Reviewed` in-catalog pill opening Blake's own modal + partial-fail/retry + 24h localStorage cache + PER SEASON/CONTINUOUS episode-counter toggle), plus an undated-entry sort fix + `UPCOMING` kicker — shipped (v1.7.2); Watched Set + Admin Form Completion — shared `franchise-fetch.js` module extraction (homepage modal + admin + CLI), `WatchedAniListIds`/`KnownAniListIds` columns so the `✓ REVIEWED` pill covers every watched entry (not just S1), unofficial-platform removal (official-only site-wide), an admin watched-set checkbox tree + a `✨ ASK` chatbot drawer (`/api/chat`, Anthropic Haiku) replacing the paste-back AI, a backfill CLI (all 44 populated), and update-log infinite-scroll (10-cap removed, restored to the v1.6.1 floor) — shipped (v1.7.3); Modal Architecture Overhaul — always-visible 3-column modal (More Info | Main | Community; proportional shrink + <900px stack; "Click for More Info" tab removed), a LARGE in-site secondary modal replacing the external-tab jump (banner/cover/synopsis/genres/tags/character grid/staff/trailer/"more like this" carousel + replace-content history-back + a "Request this anime" button), per-season markdown reviews (`season-reviews/<id>.md` + sync-emitted index + a shared XSS-safe `markdown.js` + a gold "BLAKE'S REVIEW" section + a `/admin/season-reviews` live-preview editor + `/api/season-review` CRUD + inline "✎ Edit review" deep-link), a routing split (primary id → main modal; watched-not-primary + source row → secondary w/ per-season section; non-catalog → secondary synopsis-only), a clickable character/staff tertiary detail layer (additive `CHARACTER_/STAFF_DETAIL_QUERY`), markdown rendering in the main Review/Description + the admin new-anime form (preview + B/I/link toolbar), and brand-purple markdown links across 5 surfaces — shipped (v1.7.4); Watchlist + Favorites Everywhere — secondary-modal Watchlist/Favorite save pills on any AniList entry + a non-catalog `al:<aniListId>` save schema (no Firestore rules change), account-page rendering of non-catalog saves (cover/title/format/year snapshot) + a green `✓ REVIEWED` tag + an in-site `#secondary=` deep-link route, a clickable per-episode expand (thumbnail + equal-weight multi-platform official watch links) + a sidebar WHERE TO WATCH section, signed-out saves opening the branded sign-in modal (+ an auth-modal z-index fix above the secondary/tertiary layers), `__underscore-bold__` in the shared `markdown.js`, and an AniList-sourced platforms backfill correcting all 44 reviews' streaming listings (`scripts/backfill-platforms.js` — US allowlist + name normalization + per-title Crunchyroll verification, 41 rows changed) — shipped (v1.7.5):
...
<!-- author: Code | date: 2026-06-04 -->
- v1.7.5 (2026-06-04) — Watchlist + Favorites Everywhere + per-episode/where-to-watch + platforms backfill. **Watchlist/Favorite save pills** in the secondary modal save any AniList entry (not just catalog cards) via a non-catalog **`al:<aniListId>` discriminator schema** (existing `users/{uid}/{watchlist|favorites}` collections, `{type:'anilist', aniListId, title, coverImage, format, year}` snapshot at write-time, no Firestore rules change — the homepage `watchlistSet`/`favoritesSet` snapshots give the pills their state for free). **Account page** renders non-catalog saves (cover/title/format/year, no per-row fetch) + a green `✓ REVIEWED` tag on catalog rows; clicking one opens the detail view in-site via a new **`#secondary=<aniListId>`** hash route (`account.html` loads `franchise-fetch.js` for legacy backfill). **Per-episode expand** (clickable episode rows → thumbnail + full title + equal-weight official watch links: the episode-direct link + the show's other STREAMING platforms, `MEDIA_DETAIL_QUERY.streamingEpisodes` gained `url`/`site`). A sidebar **WHERE TO WATCH** section (dedup/sorted `externalLinks` type STREAMING; STREAMING dropped from LINKS to avoid dup). **Signed-out saves** (cards + pills) open the branded sign-in modal — fixed an auth-modal z-index so it renders above the secondary (6000)/tertiary (7000) layers. **`__underscore-bold__`** added to the shared `markdown.js` (+1 test). **Platforms backfill** (`scripts/backfill-platforms.js`, dry-run + live) rewrote the Excel `Watch` column from each anime's primary-AniListId `externalLinks` (US-centric 8-platform allowlist + name normalization, Funimation/regional/TVOD filtered, per-title Crunchyroll verification for Parasyte/Death Note, Boarding School Juliet confirmed Prime-only, Hatsune Miku → "Blu-ray only"); 41 of 44 rows corrected, `WatchedAniListIds` untouched. `bump-version` stays 33 (no new versioned page). See CHANGELOG.

<!-- author: Code | date: 2026-06-04 -->
- v1.7.6 (2026-06-04) — Quick-nags PATCH before the v1.8.0 Smoothness Overhaul. **Account saved-entry routing fix:** a saved entry whose AniListId is an entry's *primary* id now opens the **main franchise modal** (Blake's review) instead of the secondary — the upgrade lives in the `#secondary=` hash handler (where `primarySlugForAniListId` is in scope), mirroring `renderRecommendations`' split; watched-not-primary + non-catalog saves stay on the secondary (the per-season review is right there). **ALSO LIKED title/format-pill overlap** fixed (the absolute corner badge now has reserved space so a wide `MOVIE` label can't sit under a short title — Mononoke flag). **Staff whitelist** 4 → 6 roles (added Series Director + Sound Director; cap stays 6). **Season-header polish** (distinct uppercase/accent/hairline on `.more-info-season > summary`). **Favicon + Apple touch icons + web manifest** — generated 16/32/180/192/512 rasters from Blake's `assets/favicon.png` source (white "R" on brand-purple), `<link>` tags across all 7 pages. Deliberately render-path-neutral (no new animations/blur) so it doesn't confound the v1.8.0 perf before/after. ROADMAP stale per-version sections restructured to the locked ladder. See CHANGELOG.

<!-- author: Code | date: 2026-06-05 -->
- v1.8.3 (2026-06-05) — **Website Identity update.** **Visitor-facing:** a per-session **welcome "door"** — Blake's own banner (optimized 2.49 MB PNG → 140 KB WebP, lazy-loaded only when shown) faded into a textured deep-purple field, a ようこそ/WELCOME kicker + Bebas wordmark + glowing **Enter**, and **outline-style anime quote bubbles** (transparent, purple text, comic outline) that fade in at random outer positions/heights and **drift very slowly up** with random lifetimes (CSS-var duration/distance, staggered JS launches, center keep-out over the card); shown once per **sessionStorage `rar:welcomed`** (close-tab/reopen re-shows; in-session nav doesn't), with a synchronous `<head>` **curtain** killing the pre-door homepage flash (JS-only → no-JS never trapped) + an exit "step through" transition; reduced-motion → instant door + one static bubble. Homepage restructured around the **BLAKE'S DEN 隠れ家** masthead (Top 10 + relocated latest drop) with an **always-on header bar** + **scroll-reveal** of sections. **Redesigned filter:** toggle **chips** (genres/tags/studios) keeping the existing checkbox read-model, **live-narrow** (keeps group headers visible — the Typhoon "blank panel" fix), a **Saved** quick-filter (watchlist ∪ favorites), a live **match-count** on Confirm, **last-filter memory** (`localStorage rar:filter:v1`), **studio dedup** by normalized key (Madhouse/Studio Madhouse, A1/A-1, etc.), and an in-flow zero-size hidden checkbox (the Typhoon **click** scroll-jump fix). **Live search** (debounced, relevance-ordered, no reshuffle) + **sparse-result centering**; **View All / wordmark** clear filters (reload still restores memory); search **✕** removed + hover-float killed. **Continue-where-you-left-off** rail (localStorage, last 6). Card-footer **shelf + gold rating pill** + **row alignment** (rating pinned bottom). Modal **"👁 Blake watched N seasons"** provenance under the vote bar. **SEO:** 192px `rel=icon` + Organization/WebSite **JSON-LD** logo. **Admin:** the ✎ Edit pill moved to share one row with the provenance under the vote bar. **Pivots from the gate plan:** anime **characters POSTPONED to v1.8.4** (kept scroll-reveal of lines/elements only); the G3b Latest-Drop **alcove** + the G5 static **den-tint** were **removed** (homepage look is being redone in v1.8.4 — both fold into the banked v1.8.4 background concept); quotes evolved bottom-lane → comic → **outline-only v4**; provenance moved card → modal; welcome became **per-session**. Tests **25** (+`welcome-splash`/`filter-search`/`g4b-fixes`/`g5` + a `welcomed` sessionStorage fixture; suite-wide storageState retired). `bump-version` 40. See CHANGELOG.

<!-- author: Code | date: 2026-06-04 -->
- v1.8.2 (2026-06-04) — **Structured review template.** **Visitor-facing:** a sticky **Kicker Rail** of jump-pills above any sectioned review (click-to-scroll + IntersectionObserver scroll-spy, **gold Overall** rating-badge nod, JP labels) on BOTH the main modal Review and the secondary BLAKE'S REVIEW, built from `extractSections` (markdown.js) and scrolling the nearest scrollable ancestor; **restored the frosted backdrops** (`.secondary-backdrop` blur(7px) + `.tertiary-backdrop` blur(6px), reverting v1.8.0's static dim — Blake accepted the Paint cost); and rebuilt the **secondary header** action row into one non-wrapping flex bar (fixed the wrap/clip/orphan). **Admin:** a **section-aware editor** on all 3 surfaces via shared `admin/section-editor.{js,css}` — Intro block + per-section blocks (title picker of the 9 with JP + Custom, body with B/I/🔗 + Ctrl/⌘ shortcuts, delete, **drag-and-drop** + ▲/▼ reorder), "Add all 9" + present-disabled picker; **lossless round-trip** via `parseReviewSections`/`compileReviewSections` in markdown.js (split on `^## ` only, everything else opaque verbatim — `render(compile(parse(x)))===render(x)`, tested), so the 44 legacy reviews load as one Intro block to carve at will. The G3 "Insert template button" **pivoted** to dedicated section fields on Blake's smoke (raw `##` was a foot-gun). G3c polish: bigger premium edit-page Review area, wrapped/softened pill rail. `tests` 14 (+`review-template`/`review-sections` specs). `bump-version` 40. See CHANGELOG.

<!-- author: Code | date: 2026-06-04 -->
- v1.8.1 (2026-06-04) — **The Admin Edit Page** (admin-only; zero visitor blast radius). New `/admin/edit` (`admin/edit.{html,js,css}`): searchable list of all 44 reviews from `animeData.js` → an editable form, reachable from the admin menu + an inline **✎ Edit review** link on the main modal (`?slug=&from=modal`). **Tiered saves** on `mode1-server.js`: Tier-1 **Save** (`PUT /api/anime/:slug` → `updateExcelRow`, hard header allowlist, row-scoped → sync) and Tier-2 **Ship live** (`POST /api/anime/:slug/ship`, SSE, reuses Mode 1's widget→bump→CHANGELOG→test→commit→deploy chain behind a branded confirm). Landed adds: interactive watched-set tree, a full-screen **live-preview** iframe of the real `#anime=<slug>` modal stack (both layers), a **change-diff confirm** before Save/Ship (full scrollable before→after; skips the write when unchanged), **Revert**, a branded **Top-10 ▲/▼ stepper**, **origin-aware** Cancel/back, the **✨ASK** Haiku drawer mounted on edit via a shared `admin/chat-drawer.js`, a per-row **🔧 Fix from AniList** (mapping extracted to a shared `scripts/lib/platform-map.js`), and a **scroll-lock** fix behind every admin overlay (shared `admin/modal-scroll-lock.js`, all 4 admin pages). New `tests/mode1-server.spec.js` boots the Mode 1 server on an isolated port and dry-run-exercises its publish + edit paths (12 tests / 9 spec files). new-anime got the "ship first, then Preview from Edit" hint. `bump-version` 40 targets. **Convergence flag:** new-anime keeps its inline ASK drawer for now — a future cleanup can migrate it onto the shared `chat-drawer.js`. See CHANGELOG.

<!-- author: Code | date: 2026-06-04 -->
- v1.8.0 (2026-06-04) — Smoothness Overhaul (round 1) + branded scrollbars. Targeted the Firefox/Gecko modal lag (Chrome was already smooth). Root cause: the secondary + tertiary modal backdrops used a live full-viewport `backdrop-filter` that **re-resolves the blur on every repaint** (hover/scroll) — Blake's Firefox Profiler showed Paint ~49%→41%, ~98% Graphics. A live backdrop-filter structurally can't be cached, so it was **removed entirely** and replaced with a premium static gradient dim (every engine stops paying the per-repaint tax). An interim Firefox-only `@supports(-moz-appearance)` layer-promote was tried + removed once the dim made it moot. Added **brand-purple scrollbars site-wide** (`::-webkit-scrollbar*` + Gecko `scrollbar-color`/`scrollbar-width`). Trimmed the trailer-iframe `allow` attr (silenced Firefox Feature-Policy warnings) + triaged the rest of Blake's console (data:-URI + hashed-bundle = extension noise, not ours; YT-embed cookie/CSP = third-party). **Documented finding:** the headless harness can't isolate the GPU backdrop-filter cost (dim ≈ backdrop-filter ≈ filter-on-static), so diagnosis rests on the re-resolve mechanism + Blake's headed profiler. **Honest verdict (Blake): felt marginal** — the bigger universal levers are measured + queued as **Smoothness round 2** below, not built here. `bump-version` stays 33. See CHANGELOG.

<!-- author: Cowork | date: 2026-06-04 -->
**Up next — THE ROADMAP, locked 2026-06-04** (Blake + Cowork + Code consultation post-v1.7.5; supersedes the per-version sections further down this file where they conflict — those get restructured at the v1.7.6 docs cascade):

| Version | Ship |
|---|---|
| **v1.7.6** (PATCH) | Quick nags: account saved-entry routing fix + More Info polish leftovers (title/format-pill overlap, staff role expansion, season-header styling) + favicon |
| ~~**v1.8.0** (MINOR) ★~~ ✅ **shipped 2026-06-04** | **Smoothness Overhaul (round 1)** — removed the live full-viewport `backdrop-filter` (per-repaint re-resolve tax) → static premium dim, branded purple scrollbars, trailer-iframe console fix, cross-engine perf investigation. Blake's verdict: marginal felt change → round 2 below. |
| **v1.8.x — Smoothness round 2** (candidate) | The **measured, un-built** universal levers from the v1.8.0 G0 profiling (deferred at Blake's "felt the same" close-out, NOT speculative): **(a) render-on-navigate caching** — the secondary/More-Info modals rebuild `innerHTML` on every Back/open (`script.js:4790/4810/4376`), re-parsing DOM + re-decoding every image; cache the built node tree / lazy-decode. **(b) Firebase SDK defer** — ~600KB of `firebase-firestore`+`firebase-auth` loads **eagerly on every page before sign-in** (the biggest universal first-load win — measured). **(c) image right-sizing** — secondary cover/banner decode `coverImage.extraLarge` (~1000px) into ~150px slots (`script.js:5027-5028`) → use `large`. **(d) perf-regression guard** — Playwright bundle-size ceiling + frame-budget assertion. **(e) esbuild minify LAST** — `script.js` 242KB→~100KB; wire into BOTH `firebase deploy` (predeploy hook) AND `mode1-server.js`'s one-click deploy, readable source stays in git + sourcemaps; ship standalone after the rest so it can't destabilize Mode 1. |
| ~~**v1.8.1** (MINOR)~~ ✅ **shipped 2026-06-04** | **Admin Edit Page** — `/admin/edit` (list + form), tiered Save (Excel allowlist→sync) / Ship live (SSE publish chain), inline ✎ on the modal, full live-preview overlay (both modals), watched-set tree, change-diff confirm, Revert, branded Top-10 stepper, origin-aware nav, ✨ASK on edit + per-row 🔧 Fix-from-AniList (shared `platform-map.js`), scroll-lock across all 4 admin pages, Playwright Mode-1 spec, new-anime "preview after ship" hint. Convergence flag: new-anime's inline ASK drawer can later move onto the shared `chat-drawer.js` |
| ~~**v1.8.2** (MINOR)~~ ✅ **shipped 2026-06-04** | **Structured review template** — section pill rail + scroll-spy (gold Overall, JP labels) on both review surfaces; section-aware editor on all 3 admin surfaces (drag reorder, B/I + Ctrl/⌘ shortcuts, lossless round-trip); restored frosted backdrops + fixed secondary header row. The "Insert template button" pivoted to dedicated section fields on Blake's smoke. |
| ~~**v1.8.3** (MINOR)~~ ✅ **shipped 2026-06-05** | **Website Identity update** — per-session welcome "door" (Blake's banner + slow outline anime-quote bubbles + flash-killing curtain), BLAKE'S DEN homepage restructure + persistent header + scroll-reveal, redesigned filter (chips / live-narrow / Saved / match-count / memory / studio dedup + the Typhoon type-AND-click fixes), live search + sparse centering, continue-rail, card-footer accents + row alignment, modal "watched N seasons" provenance, SEO icon/JSON-LD, admin edit-pill row. Pivots: characters → v1.8.4; alcove + den-tint removed (→ v1.8.4 background concept); quotes → outline v4. |
| ~~**v1.8.4** (MINOR) ★~~ ✅ **shipped 2026-06-06** | **Discovery & Blend** — three adjacent surfaces (Blake's Den / **For You** / **Discover**), one blended card shell + `reviewed` flag (gold "Reviewed by Blake" pins lead; outside cards get a NOT-REVIEWED sticker + community score + the secondary modal's "Request this anime"); 3 additive flat AniList queries on the L2 cache (101922-canary-tested) + a fresh-per-log-on shuffle; the **real nav** (Den/For You/Discover + sliding gold marker) + a **composed homepage** (AIRING NOW strip · For-You teaser · Top10+Latest-Drop side-by-side); the **constellation veil** (per-surface density over the kept city backdrop) + an **animated line-pulse**; the update log re-homed into the welcome door + per-update tier labels; a once-per-visit More-Info deep-dive hint; an admin **Quotes** page (drag/▲▼ reorder, search, dupe/length checks, ✨ASK) writing a public `quotes.json` the door fetches. **Pivots from the plan:** characters KILLED (the veil replaced them); background shipped as Dir-2 "Constellation hush" (Blake's pick) over the kept backdrop. `bump-version` 47. |
| **v1.9.0** (MINOR→MAJOR) ← **next** | Community/Account/User overhaul (comments incl. per-season, community reviews, notifications, DM inbox, account redesign) + Cloud Functions (notification pruning, cascade deletes) + privacy notice. The big user-data design-study ship (Blake: "asap") |
| **v1.9.5** (MINOR) | Site-wide UI overhaul + search tuning; "AniList tab on cards" folds into the card redesign |
| **v2.0.0** (MAJOR) | Mobile overhaul |

**Closed at the 2026-06-04 consolidation:** per-episode reviews/"full course", one-click AI integration (shipped v1.7.3), one-click full automation (deploy-confirm stays as a safety gate), AniList per-episode scores check (answered: none exposed), Re:Zero dedupe (shipped v1.7.2).
...
<!-- author: Code | date: 2026-06-04 — RESTRUCTURED at v1.7.6: the v1.8.0 slot is now the Smoothness Overhaul (see the locked ladder above). This "at-a-glance verified data" scope FOLDS INTO v1.9.5's card redesign rather than a standalone tab. Scope kept below as reference for v1.9.5. -->
### (Superseded → folds into v1.9.5) AniList data at-a-glance on cards
...
<!-- author: Cowork | date: 2026-06-04 -->
<!-- author: Code | date: 2026-06-04 — RENUMBERED at v1.7.6: this is now **v1.9.0** per the locked ladder above (was v1.8.5). Scope unchanged. -->
## v1.9.0 — Community + Account UI/UX Overhaul

Dedicated overhaul ship surfaced 2026-06-04 during v1.7.4 close-out. Slotted before the v2.0.0 mobile work because the community/account surfaces should be in their final form before the mobile compatibility audit targets them. Likely large enough to be MAJOR depending on scope growth.
...
<!-- author: Code | date: 2026-06-04 — RENUMBERED at v1.7.6: this is now **v2.0.0** per the locked ladder above (was v1.9.0). Mobile = the 2.0 milestone. -->
## v2.0.0 — Mobile compatibility overhaul
