...
## ⚡ READ-FIRST
- **Live at v1.10.1** (the Community Hub + the upload hotfix, 2026-06-12). **v1.10.2 is STAGED (2026-07-02, deploys on Blake's go):** the Creator profile at member parity + the full account nav + the member-reported prod-bug fixes + the working Mode-1 launcher. **The next ship is the mega-run** (DMs & Dreams + Discover + UI polish + the v2.0.0 mobile overhaul — see `docs/NEXT.md`; one known carry-over: review-targeted notification deep-links don't always scroll-to-highlight yet).
