...
**Up next:** v1.7.4 — in-site secondary modal for franchise entries (replaces the open-externally jump; benefits from v1.7.3 since the `✓ REVIEWED` pills now work across all watched seasons). After that: v1.8.0 — AniList tab on cards (at-a-glance verified-source data).
