...
// v2.3.4 — ALWAYS/OPTIONAL come in too: the generated file now ships the field
// list it emitted, so the site's runtime top-up copies a live doc by the same
// names rather than a second hand-kept list that could drift.
const { parseAnimeData, renderBody, checkShrink, ALWAYS, OPTIONAL } = require('./lib/catalog-model');
...
(async () => {
  ...
  // ---- cover tripwire (v2.3.4) -------------------------------------------
  // Nothing used to check that assets/<image> existed. The filename is emitted
  // verbatim from the Firestore doc, so a typo or a not-yet-fetched cover shipped
  // a broken image with no warning anywhere — Blake would only find it by looking
  // at the live site. Same voice and same --force escape as the shrink tripwire.
  //
  // An entry with an AniListCover is a WARNING, not a refusal: the card falls back
  // to that URL, so the site is correct, just slower than a local file. An entry
  // with neither is a refusal — that one really would render the placeholder.
  console.log(`\n${C.b}Cover art${C.x}`);
  const missing = [];
  const remoteOnly = [];
  for (const a of docs) {
    if (!a.image) { missing.push(`"${a.Title}" has no image filename at all`); continue; }
    if (fs.existsSync(path.join(CV, 'assets', a.image))) continue;
    if (a.AniListCover) remoteOnly.push(`"${a.Title}" → assets/${a.image}`);
    else missing.push(`"${a.Title}" → assets/${a.image} is missing, and it has no AniList cover to fall back on`);
  }
  for (const w of remoteOnly.slice(0, 8)) {
    console.log(`  ${C.y}!${C.x} ${w} ${C.d}(not uploaded — the site will use the AniList image)${C.x}`);
  }
  if (missing.length) {
    console.log(`\n${C.r}${C.b}⛔ PUBLISH REFUSED — these would ship a broken image:${C.x}`);
    for (const m of missing) console.log(`  ${C.r}✗${C.x} ${m}`);
    if (!FORCE) {
      console.log(`\n${C.d}Put the file in assets/ (400x600 PNG, like the others) and re-run.`);
      console.log(`If it is deliberate, re-run with --force.${C.x}`);
      process.exit(1);
    }
    console.log(`\n${C.y}--force given: proceeding despite ${missing.length} missing cover(s).${C.x}`);
  } else if (!remoteOnly.length) {
    console.log(`  ${C.g}✓ every cover is on disk${C.x}`);
  }

  // ---- render -----------------------------------------------------------
  // v2.3.4 — the header now carries two REAL VALUES, not just comments, because
  // the site tops itself up at runtime (script.js catalogTopUp) and needs to ask
  // Firestore "anything changed since this file was built?".
  //   RAR_CATALOG_PUBLISHED_AT — the build stamp, i.e. what "since" means.
  //   RAR_CATALOG_FIELDS       — the emitted field list, so the top-up copies a
  //                              live doc using the SAME names this generator
  //                              emits. Shipping the schema with the data is what
  //                              stops the two drifting apart silently.
  // Both sit BEFORE the `const animeData = [` marker, so they land in the header
  // half of splitAnimeDataFile() and the byte-exact round-trip test is untouched.
  const stamp = new Date().toISOString();
  const header = [
    '// AUTO-GENERATED by scripts/catalog-publish.js — do not edit by hand.',
    '// Source of truth: the catalog store (Firestore)',
    '// Last publish: ' + stamp,
    'const RAR_CATALOG_PUBLISHED_AT = ' + JSON.stringify(stamp) + ';',
    'const RAR_CATALOG_FIELDS = ' + JSON.stringify([...ALWAYS, ...OPTIONAL]) + ';',
    '',
  ].join('\n') + '\n';
