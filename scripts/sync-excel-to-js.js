...
// v2.1.0 cloud-migration phase 2 — the body renderer moved to
// scripts/lib/catalog-model.js so there is exactly ONE definition of the
// animeData.js format. The Excel path (here) and the cloud Publish path
// (scripts/catalog-publish.js) now emit through the same function, which is
// what makes "Excel and the database produce identical output" a fact rather
// than a coincidence. Output is unchanged: the entry formatting, the field
// order, and the v1.7.3 watched-set arrays all live in renderBody() verbatim.
const { renderBody } = require('./lib/catalog-model');
...
function renderJsFile(animes) {
  ...
  return header + renderBody(animes);
}
