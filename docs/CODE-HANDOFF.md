...
<!-- STATE (2026-06-06): v1.8.3 is LIVE in prod (APP_VERSION 1.8.3, HEAD d318334). v1.8.4 (Discovery & Blend) is MID-BUILD: gates G1 THROUGH G7 are ALL DONE + green but ENTIRELY UNCOMMITTED in the working tree (one big ship — it all commits at the SWEEP). npm test = 82 (the floor). The NEXT thing is the SWEEP (commit + bump + prod), UNLESS Blake opts into the optional G8 (Quotes-admin) first — his call. docs/SHIP-PROMPT.md is ALREADY staged by Cowork and READY TO READ (it has the next gate). Read the "YOU ARE HERE" snapshot below FIRST, then docs/SHIP-PROMPT.md, then docs/SHIP-OUTPUT.md (last report = G7), then recon the real files. ⚠️ ULTRACODE TOGGLES: it was ON for G1-G6 (author Workflows for substantive gates — the design-panel + adversarial-review pattern caught real bugs and gave great copy) and OFF for G7 (worked directly). CHECK the ultracode system-reminder at session start and follow the Workflow opt-in rule accordingly. -->
...
⚠️ **Read the "⚠️ YOU ARE HERE" snapshot (next heading) FIRST** — it has the exact current state (v1.8.3 LIVE; v1.8.4 MID-BUILD, gates G1→G3e done + uncommitted, Gate 4 "For You" staged). The mannerisms / 12-gate workflow / commit discipline / traps that bite hardest are all further down and still accurate — they ARE how we work, read them before you touch code.
...
## ⚠️ YOU ARE HERE (snapshot — 2026-06-06, v1.8.3 LIVE · v1.8.4 BUILD COMPLETE G1→G7, SWEEP NEXT)

> ⚠️ The detailed per-gate "what's built" text FURTHER DOWN this file is HISTORICAL (written in the G1–G3e era). THIS snapshot + `docs/SHIP-OUTPUT.md` (the G7 report) + the REAL files are authoritative. Don't trust the old "Gate 4 staged" / "G1→G3e" lines below.

**v1.8.3 is LIVE in prod** (`APP_VERSION="1.8.3"`, realanimereviews.com, HEAD `d318334`). **v1.8.4 (Discovery & Blend) is BUILD-COMPLETE — gates G1 through G7 are ALL done + green — but ENTIRELY UNCOMMITTED in the working tree.** That's correct + by design: v1.8.4 is ONE BIG SHIP; it all commits at the **SWEEP**. **Do NOT commit mid-ship. Do NOT bump the version yet** (the bump is a sweep step). **`npm test` = 82 (the floor).** `node scripts/bump-version.js --check` = "all 40 agree on 1.8.3".

**WHAT THE NEXT CODE DOES:** read `docs/SHIP-PROMPT.md` (Cowork has it STAGED + ready — it's the next gate's instructions: either the **SWEEP** or, if Blake opted in, the optional **G8 (Quotes-admin)** first). Then `docs/SHIP-OUTPUT.md` (the G7 report). Then recon the real files. Then follow the staged prompt's gate number.

**`git status` (expected):** modified `account.html · card-render.js · franchise-fetch.js · index.html · script.js · style.css · suggest.html` + 3 modified test specs (`g4b-fixes`, `homepage-loads`, `modal-leak-check`) + the rolling/Cowork docs; untracked = `scripts/anilist-discovery-canary.js` + the new test specs (`discover-*`, `discovery-*`, `foryou-surface`, `gate5-nav-home`, `gate6-veil`, `card-keyboard-a11y`, `welcome-3e`) + `docs/COWORK-STYLE.md`. **At the sweep:** `git add -A` → `git restore --staged` the 7 Cowork excludes (they show modified — Cowork manages them) → confirm absent from `--cached` → ONE commit `--author="Blake Wolters <196413457+joewolters@users.noreply.github.com>"`, ZERO trailers → preview → **prod ONLY on Blake's explicit "ship it."**
