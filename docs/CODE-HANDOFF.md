...
## Right now (snapshot — 2026-06-04, v1.8.1 SHIPPED + LIVE)

**v1.8.1 (Admin Edit Page) is the LAST shipped version — LIVE in prod** (commit `d60c437`, `APP_VERSION="1.8.1"`, realanimereviews.com; deployed 2026-06-04). HEAD == origin/main == prod. Working tree is clean except the rolling docs (this file + `docs/SHIP-OUTPUT.md`, next docs commit). **If you're the next Code: read `docs/SHIP-PROMPT.md` for the staged gate — next ship is v1.8.2 (Structured review template).**

### What v1.8.1 shipped (the Admin Edit Page — all live, in commit `d60c437`)
**The edit page** (`admin/edit.{html,js,css}`) — searchable list of all 44 reviews loaded **straight from `animeData.js`** (browsing needs NO server) → an editable form (Rating/Seasons/Genre/Studio/Platforms/Trailer/Tags/Top10Rank/Description + a **live-markdown Review preview** + the **interactive watched-set tree** ported from new-anime, pre-checked from the row's `WatchedAniListIds`). **Title is read-only** (slug = `slug(Title)`; editable title needs a stored-slug schema change — future op). Reachable from the `admin-fab` "Edit a Review" item AND an inline **"✎ Edit review"** pill on the main modal's badge row (gated by `window.__rarIsAdmin`; the link carries `?slug=&from=modal` for origin-aware return).

**Two save tiers (`mode1-server.js`):**
- **Tier-1 "Save" — `PUT /api/anime/:slug`** (`?dryRun` supported) → lock-check → `backupExcel` → **`updateExcelRow(slug, fields, {dryRun})`** (hard `EDITABLE_HEADERS` allowlist, row found by `slugify(Title)`, never Title/AniList-fields/KnownAniListIds/other rows; dryRun skips the write) → `npm run sync`. Client `slugify` is a byte-copy of the server's.
- **Tier-2 "Ship" — `POST /api/anime/:slug/ship`** (SSE) → reuses Mode 1's chain (widget → bump PATCH → CHANGELOG → `npm test` → scoped commit+push → deploy). Branded confirm + **save-first** (can't ship stale). The confirm IS the prod go-signal.

**The edit tooling (G2b/G3/G4/G4b — all landed):** a **change-diff confirm** before Save/Ship (`computeChanges`/`renderDiff`; full scrollable before→after; **skips the PUT when nothing changed**; the watched set compared as a SET), **Revert** (snapshot reset incl. the tree via `initialWatchedChecked`, no refetch), a branded **Top-10 ▲/▼ stepper** (`stepTop10`, native spinner hidden), **origin-aware** Cancel/back/Esc (`formOrigin` from `?from=modal` → `returnToOrigin` → `index.html#anime=<slug>`; list → list), a full-screen **live-preview** iframe of the real `#anime=<slug>` modal stack, the **✨ASK** Haiku drawer mounted via a NEW shared **`admin/chat-drawer.js`** (`window.RarChatDrawer.mount({getSession})`; new-anime KEPT its inline copy — convergence flagged), and a per-row **🔧 Fix from AniList** (`POST /api/anime/:slug/platforms` → shared **`scripts/lib/platform-map.js`** `proposePlatformsForRow`; premium amber flag pills; visible offline/error feedback — no silent no-op).

**Cross-cutting:** **`admin/modal-scroll-lock.js`** (NEW, shared) — a MutationObserver locks page scroll behind any admin overlay; loaded on ALL 4 admin pages (edit/new-anime/season-reviews/suggestions). New **`tests/mode1-server.spec.js`** boots the Mode 1 server on `MODE1_PORT` and dry-run-exercises `/api/submit` (SSE step order) + `PUT /api/anime/:slug?dryRun` (**npm test = 12 tests / 9 spec files** now). `scripts/backfill-platforms.js` refactored to require the shared `platform-map.js` (byte-identical). `bump-version` = **40 targets**. new-anime got a "ship first, then Preview from Edit" hint (closed the G3 open question).

### What v1.8.1 STILL needs
- **Nothing — it's shipped + live.** Next ship is **v1.8.2 (Structured review template)** — see ROADMAP/NEXT. The only carry-forward is the **new-anime ASK-drawer convergence** (optional cleanup: migrate new-anime's inline chat onto `admin/chat-drawer.js`).
...
4. **Read `docs/SHIP-OUTPUT.md`** — what just happened (currently: v1.8.1 production deploy DONE — LIVE at `d60c437`, APP_VERSION 1.8.1).
...
**v1.8.1 (Admin Edit Page) is LIVE in prod** (`d60c437`, deployed 2026-06-04, APP_VERSION 1.8.1; on v1.8.0 `38a4baf` / v1.7.6 `e648f57` / v1.7.5 `b085ec1`) — a premium admin surface to edit existing reviews end-to-end (no hand-editing Excel): the `/admin/edit` page (searchable list → form straight from `animeData.js` + the watched-set tree + live-markdown Review preview), **Tier-1 `PUT /api/anime/:slug`** (`updateExcelRow` — row by `slugify(Title)`, hard `EDITABLE_HEADERS` allowlist, `?dryRun` no-write, temp-copy-proven row-scoped → sync), **Tier-2 `POST /api/anime/:slug/ship`** (SSE, reuses Mode 1's widget→bump→CHANGELOG→test→commit→deploy chain, branded confirm + save-first), an inline **"✎ Edit review"** on the main modal (`?slug=&from=modal`), a full-screen **live-preview** iframe of the real `#anime=<slug>` modal stack, a **change-diff confirm** before Save/Ship (skips the write when unchanged; watched set compared as a SET), **Revert** (snapshot reset), a branded **Top-10 ▲/▼ stepper**, **origin-aware** Cancel/back, the **✨ASK** Haiku drawer mounted via a NEW shared `admin/chat-drawer.js`, a per-row **🔧 Fix from AniList** (`POST /api/anime/:slug/platforms` → NEW shared `scripts/lib/platform-map.js`, premium flag pills, visible offline/error feedback), a NEW shared `admin/modal-scroll-lock.js` (scroll-lock behind every overlay, all 4 admin pages), a NEW `tests/mode1-server.spec.js` (boots the server on `MODE1_PORT`, dry-run-tests `/api/submit` SSE + `PUT ?dryRun`; **npm test = 12 / 9 spec files**), and the new-anime "preview after ship" hint; `bump-version` is **40 targets**; **live-verified** (/ 200 + APP_VERSION 1.8.1, /admin/edit 200, new assets 200, leak checks 404); **convergence flag:** new-anime kept its inline ASK drawer (optional future migration onto `chat-drawer.js`). The project lives in `Current Version/` (CWD is the parent — prefix paths); workflow is the 12-gate model with the SHIP-PROMPT/SHIP-OUTPUT/HANDOFF rolling trio (SHIP-OUTPUT rewritten every response, follow the STAGED gate number which Cowork sometimes renumbers), commits authored Blake via per-commit `--author=` with ZERO trailers and the 7 Cowork docs restore-staged out, prod only on explicit "ship it"; durable architecture: the shared `markdown.js` (5 consumers, single-source — don't re-duplicate), the 3-layer modal system (primary mounted → secondary `#secondary-layer` z6000 → tertiary `#tertiary-layer` z7000), per-season reviews (`season-reviews/<id>.md` static-deployed + `/api/season-review` LOCAL CRUD + `/admin/season-reviews` editor needing `npm run mode1`), the routing split (`primarySlugForAniListId`+`isWatchedAniListId`; WatchedAniListIds POPULATED across all 44), and now the shared `franchise-fetch.js` + `platform-map.js` + `chat-drawer.js` + `modal-scroll-lock.js`; recurring traps are phantom-drift + gate-prompt arithmetic/assumption errors (re-derive the gate's numbers yourself — e.g. its "9 specs" meant 9 FILES not 9 tests, `PORT` was hardcoded not env-driven) and the **`sync-excel-to-js.js` parse-vs-serialize split** (any new animeData field = edit BOTH halves); use live AniList node queries against the **101922 canary** for ground-truth recon and the **Grep TOOL** (not bash) for smart-quotes; **NEXT = v1.8.2 Structured review template** (9 `##` sections + "Insert template" admin button + jump-pill render, pairs with the edit-page infra), then v1.8.3 identity, Smoothness round 2 (Firebase-defer ~600KB / render-on-navigate / image right-size / minify-last / perf guard), v1.9.0 community, v1.9.5 UI, v2.0 mobile. ⚠️ Perf lesson: headless can't measure GPU backdrop cost — use the headed Firefox Profiler; don't reintroduce a live full-viewport backdrop-filter.
