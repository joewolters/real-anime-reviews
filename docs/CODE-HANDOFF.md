...
## Right now (snapshot — 2026-06-04, end of the v1.7.6 session)

**v1.7.6 — Quick-nags PATCH — is LIVE in production** (commit `e648f57`, deployed 2026-06-04, `APP_VERSION="1.7.6"` on realanimereviews.com). Ran as Gate 1 (build all 5 nags) + the compressed sweep (docs → audits → commit → preview → prod). Live-verified: version flipped 1.7.5→1.7.6, `/assets/favicon-32.png` + `/assets/apple-touch-icon.png` + `/site.webmanifest` all 200, `.env` + the 4 rolling/cowork docs all 404. `e648f57` == origin/main == preview channel == prod.

**Status: v1.7.6 is shipped + live-verified.** **Next ship is v1.8.0 — Smoothness Overhaul (PROPOSE-FIRST gate 0).** The perf recon is already done: see the **2026-06-04 roadmap-consultation SHIP-OUTPUT** (it's been overwritten since, but the content is summarized below) — top-5 offender table + the cheap-win blur de-stack strategy. **If you're the next Code:** read `docs/SHIP-PROMPT.md`.

**What v1.7.6 added (small):** the account saved-entry routing fix lives in the **`#secondary=<aniListId>` hash handler** (script.js, ~`:5975`) — it now checks `primarySlugForAniListId(id)` and upgrades a primary-id save to the MAIN franchise modal (`openModal`), else falls through to `openSecondaryModal` (watched-not-primary + non-catalog). The fix is in the handler, NOT account.js, because the routing helpers are IIFE-scoped. Plus: ALSO LIKED title/badge overlap fix (`.more-info-recommendations .more-info-body{padding-right:50px}`), staff whitelist 4→6 (`renderStaffCredits` `:1215`, +Series Director +Sound Director), season-header polish (`.more-info-season > summary`), and a **favicon set** (`assets/{favicon-16,favicon-32,apple-touch-icon,icon-192,icon-512}.png` from the 1254px `assets/favicon.png` source + `site.webmanifest`, `<link>`s in all 7 page heads via root-absolute `/assets/`). ⚠️ Favicon gotcha: `convert` on this box is the **Windows NTFS tool, NOT ImageMagick** — resize via PowerShell `System.Drawing` (HighQualityBicubic). `bump-version` stays **33**.

**What v1.7.5 added (still current architecture):** secondary-modal **Watchlist/Favorite pills** via the non-catalog **`al:<aniListId>` discriminator** in `users/{uid}/{watchlist|favorites}` (no Firestore rules change; homepage `watchlistSet`/`favoritesSet` give pill state free via `.has('al:'+id)`). Account renders non-catalog saves (`renderSaved` branches on `data.type==='anilist'`). Per-episode **in-row expand** (`renderSecondaryEpisodes`) + **WHERE TO WATCH** (`renderSecondaryPlatforms`), both off `detail.externalLinks` type STREAMING. Signed-out saves → `openAuth('signin')` (auth z-index 8000/7950). `markdown.js` has `__bold__`. `scripts/backfill-platforms.js` (dry-run + live) corrects the `Watch` column from AniList.

**Uncommitted at session end (next docs commit):** `docs/CODE-HANDOFF.md` (this file) + `docs/SHIP-OUTPUT.md`. Code deliverables + rolling docs are committed in `e648f57`; live code == `e648f57`.
...
## What's next — v1.8.0 Smoothness Overhaul (the queued ship) + later

**v1.8.0 — Smoothness Overhaul (PROPOSE-FIRST gate 0; Blake's stated priority).** The perf recon is DONE (the 2026-06-04 roadmap consultation) — the next Code should re-confirm the offenders live but the diagnosis is solid. **Top-5 offenders (with evidence):**
1. **Stacked full-viewport `backdrop-filter` blurs** — `style.css` `.modal-overlay` blur(6px) (~`:4522`) stays active *under* `.secondary-backdrop` blur(7px) (~`:5301`) whenever the secondary modal is open (primary stays mounted). Two viewport blurs re-painted on **every hover/click repaint**. **~60-80% of the modal lag.** **CHEAP WIN:** while the secondary is open, turn OFF the primary `.modal-overlay` blur (not visible through the opaque secondary) + drop `.secondary-backdrop` to a plain dim → most smoothness back for a few lines + a class toggle.
2. **16 hover rules animate `filter`** (`transition: … filter` at `style.css:189,987,1171,1545,1871,1936,1971,4275,4356,5144,…`) — repaint loops; swap to `opacity`/`transform` (composite, no repaint).
3. **Full `innerHTML` teardown+rebuild on every modal navigation** (`script.js` secondary `:4790/4810`, More Info panel `:4376-4415`) — re-parses + re-decodes every image. Cache the built DOM / lazy-decode.
4. **242KB unminified `script.js`** (+179KB `style.css`) — add an esbuild minify step (durable infra, ~242KB→~100KB).
5. **`extraLarge` covers** decoded into ~150px slots (`script.js:5027-5028`) — use `large`.
Suggested gate plan: G0 profile (Code reasons + Blake runs DevTools Performance) → G1 blur de-stack (cheap win) → G2 render-on-navigate → G3 minify + image right-size → G4 re-measure + a perf-regression guard (Playwright bundle-size/frame-budget). ~6-10h. **Constraint:** this is the perf ship — measure before/after; the v1.7.6 nags were deliberately render-path-neutral so the baseline is clean.
