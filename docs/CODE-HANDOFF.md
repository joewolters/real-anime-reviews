...
<!-- STATE (2026-06-06): v1.8.4 (Discovery & Blend) is LIVE in prod (APP_VERSION 1.8.4, realanimereviews.com, HEAD 88b2976 on main, in sync with origin). The 10-gate ship (G1->G8b) is built + swept + committed (ONE Blake-authored commit, zero trailers) + pushed + deployed; live-verified. npm test = 84 (the floor); bump-version = 47 targets. NEXT = v1.9.0 Community/Account/User overhaul — its OWN gate-0 design study (Cowork+Code brainstorm; Blake wants it "asap"). ⚠️ Cowork HAS STAGED the v1.9.0 gate-0 SHIP-PROMPT — it is ready to read NOW. Read the "YOU ARE HERE" snapshot below FIRST, then docs/SHIP-PROMPT.md (the staged v1.9.0 gate-0 brainstorm — follow ITS gate number), then docs/SHIP-OUTPUT.md (last report = the v1.8.4 prod deploy), then ROADMAP/NEXT (the v1.9.0 scope + my banked v1.8.4 knobs + my v1.9.0 wants), then recon the real files. ⚠️ ULTRACODE was toggled per-gate across v1.8.4: ON for the design/recon/review gates (the design-panel + adversarial-review Workflow pattern caught real bugs + gave great copy), OFF for mechanical gates (sweep/fixes). CHECK the ultracode system-reminder at session start and follow the Workflow opt-in rule accordingly. -->
...
⚠️ **Read the "⚠️ YOU ARE HERE" snapshot (next heading) FIRST** — it has the exact current state (v1.8.4 LIVE; v1.9.0 Community/Account is next, a gate-0 design study). The mannerisms / 12-gate workflow / commit discipline / traps that bite hardest are all further down and still accurate — they ARE how we work, read them before you touch code.
...
## ⚠️ YOU ARE HERE (snapshot — 2026-06-06, v1.8.4 LIVE · v1.9.0 Community/Account NEXT, gate-0 design study)

> **UPDATE 2026-06-06 (gates P0/P1/1/2 done):** gate-0 study shipped, Blake answered all 7 Qs on the recommendations, and **P0 + P1 + Gate 1 + Gate 2 are APPLIED + verified (NOT committed yet — checkpoint commit is after Gate 3).** **Gate 2 (CF core):** `onCommentVote`/`onReviewVote` (`onDocumentWritten` → `FieldValue.increment()` counts + server-sourced notification + `eventId` idempotency via a new internal `cfProcessed/{eventId}` collection + mute-at-source) and `pruneNotificationsOnCreate` (trim to 10 + `expiresAt` TTL); pure logic in `functions/lib/{votecounts,notify,prune}.js`. Tests: **`test:functions` 18/18** (pure units) · **`test:cf` 5/5** (emulator: server-sourced name beats a forged one, 5 concurrent votes = exact count, 11th prunes, mute suppresses, unvote decrements) · `test:rules` still 35/35. ⚠️ **These CFs are EMULATOR-VERIFIED, NOT deployed** — they'd double Blake's bell + counts against the still-live client path, so they deploy atomically at the gate-6 cutover (only `ping` is live). **JDK 21 is now PERMANENT** at `C:\Users\Owner\jdk-21.0.11+10` (persistent user `JAVA_HOME`/`PATH`). **Next: Gate 3 = cascade deletes (`onUserDelete` incl.) + rate-limit + `aggregateSuggestionCounts`, then the first CHECKPOINT COMMIT** (P0→3, one Blake-authored commit, 7 Cowork excludes restore-staged out, NO deploy). Earlier-state note below ⤵ Contract = **`docs/DATA-MODEL.md`**; `docs/ARCHITECTURE.md` rewritten to ground truth; study distillation + 7 answers in `docs/NEXT.md`. **P1:** `functions/` gen-2 scaffold (no-op `ping` + `maxInstances:10`), `firebase.json` `functions`+`emulators` blocks, `npm run test:functions` (2/2). **Blaze is ON** — `ping` is **LIVE + verified** at `https://us-central1-real-anime-reviews.cloudfunctions.net/ping` (`pong`, 200); Artifact Registry cleanup policy set (>1-day images). **Gate 1:** full `firestore.rules` rewrite (all new collections + every security fix H1–H5/M1–M6/L2–L3 binding — staging banner at top of the file), `firestore.indexes.json` populated, and **`npm run test:rules` = 35/35 green in the Firestore emulator** (21 hostile-denied + 14 happy). ⚠️⚠️ **The new `firestore.rules` is STAGED, NOT DEPLOYED** — it intentionally breaks the live client (H1 count writes, H3 notif writes); production still runs the OLD rules. **Do NOT `firebase deploy --only firestore` (or a bare `firebase deploy`) until the gate-6 CF cutover** or live voting + the bell break. ⚠️ **JDK 21+ required** for the emulator (firebase-tools rejects Java <21; I used a portable Temurin 21 in Temp — Blake needs a real JDK 21 install for `npm run test:rules`/`npm run emulators`). **Two contract corrections this gate** (both in DATA-MODEL.md): per-season comments REUSE `comments/al:{id}`/`reviews/al:{id}` (no separate `seasonComments` collection); tightening `users/{uid}` get is DEFERRED (3 live sites cross-read it — script.js:4125/4898/5124). (Gate 2 is now DONE — see the lead note at the top of this block; next is Gate 3, then the first checkpoint commit.)

> ⚠️ The detailed per-gate "what's built" text FURTHER DOWN this file is HISTORICAL (the v1.8.3 / v1.8.4-build era). THIS snapshot + `docs/SHIP-OUTPUT.md` (the v1.8.4 prod-deploy report) + ROADMAP/NEXT + the REAL files are authoritative.

**v1.8.4 (Discovery & Blend) is LIVE in prod** (`APP_VERSION="1.8.4"`, realanimereviews.com, HEAD `88b2976` on `main`, in sync with origin). The 10-gate ship (G1→G8b) was built, swept, committed as **ONE Blake-authored commit** (`--author="Blake Wolters <196413457+joewolters@users.noreply.github.com>"`, **ZERO trailers verified**, the 7 Cowork excludes restore-staged out + `COWORK-STYLE.md` left untracked), pushed `d318334..88b2976 main`, preview-verified, and deployed to prod on Blake's **"ship it everything looks good."** Live-verified: APP_VERSION 1.8.4, `/admin/quotes` + `/quotes.json` 200, the veil + tiered update-log render, all leak checks 404. **`npm test` = 84 (the floor).** `bump-version --check` = "all **47** agree on 1.8.4".

**WHAT v1.8.4 SHIPPED (live function names — for building ON it):** the 3 surfaces (Den / For You / Discover) via `setActivePlace`/`moveMarker` + the sliding gold marker; the blended cards (`createDiscoveryCard` — his 44 gold-pinned via `createCard(a,{pinned:true})`, outside cards NOT-REVIEWED + `openSecondaryModal`); the data layer (`franchise-fetch.js`: `searchMediaList`/`fetchTrendingList`/`fetchAiringList`, 101922-canary; caches `fetchTrendingCached`/`fetchAiringCached` on `window.rarDiscovery`); the taste engine (`computeTasteProfile`/`buildForYou`); the composed homepage (`fillHomeAiring`/`buildHomeForYou`/`lazyFillOnView`); the **constellation veil** (`body::after`, `--veil-w0/w1/w2` @property, `data-surface` per surface set ONLY in `setActivePlace`) + the **animated pulse** (`#veil-pulse` masked ring, z-ladder city `-3`/veil `-2`/pulse `-1`, gated behind a JS-added `html.veil-pulse-active` so no-JS/reduced-motion keeps the static lit veil; index.html only); the welcome-door **quotes** (now public `quotes.json` + `WELCOME_QUOTES_FALLBACK` in script.js); the **Quotes admin** (`admin/quotes.*` + `/api/quotes` on mode1 + `scripts/lib/quotes-store.js`); the update-log **tier rails** (`.vs-head`/`data-tier`/`.vs-tier`, honest tier from the CHANGELOG semver token); the More-Info **deep-dive hint** (`deepDiveHintFor`, `.mi-hint-pill`).

**WHAT THE NEXT CODE DOES:** **v1.9.0 (Community/Account/User overhaul) is NEXT** and is its OWN **gate-0 design study** (Blake: "asap") — a Cowork+Code brainstorm BEFORE any build (comments incl. per-season, community reviews, notifications, DM inbox, account redesign + Cloud Functions + privacy notice; see the ROADMAP v1.9.0 row + the NEXT.md v1.8.5/community scope). **⚠️ Cowork HAS STAGED the v1.9.0 gate-0 SHIP-PROMPT — read it (`docs/SHIP-PROMPT.md`, ready NOW).** Read order: this file → `docs/SHIP-PROMPT.md` (the staged v1.9.0 gate-0 brainstorm) → `docs/SHIP-OUTPUT.md` (the v1.8.4 prod-deploy report) → `ROADMAP.md`/`docs/NEXT.md` (the v1.9.0 scope + my banked knobs + "Code's wants for v1.9.0") → recon the real files → follow the staged prompt's gate number. **The working tree is CLEAN of code** (everything is in `88b2976`); the only dirty files are the rolling/Cowork docs (this file + SHIP-OUTPUT + the 7 Cowork excludes) — normal, they roll/are Cowork-managed.

**v1.8.4 banked knobs + near-term riders (full detail in NEXT.md "CODE'S BANKED IDEAS"):** For-You title alternates (one-string swap at the `#for-you-view` h2 + aria-label) · veil-pulse tuning knobs (`veilLinePulse 16s` / `.vp-glow top:40%` / peak `opacity 0.82`) + the account/suggest pulse extension (the pulse is index.html-only — add the `#veil-pulse` div + the ~10-line `initVeilPulse` IIFE there) · the showcase bottom-align knob (Blake "may leave for now") · "N people requested" → a v1.9.0 Cloud-Function count aggregate · "Blake's Constellation" static pre-baked SVG behind the Den Top-10. **NEAR-TERM riders:** an About-Me/Credits footer overhaul (Blake) · More-Info nudge SHIPPED ✓ · update-log redesign SHIPPED ✓ · the Mode-1 desktop launcher still parked. ⚠️ `docs/SKILLS/widget-update-skill.md` (Cowork-managed) carries the new `.vs-head`/`data-tier`/tier-label authoring shape — future ships author the update log that way.

### The v1.8.4 gate map (DONE) + what each shipped — the live function names
- **G1 data ✓** — `franchise-fetch.js` (additive): `searchMediaList`/`fetchTrendingList`/`fetchAiringList` (3 flat queries, `type:ANIME isAdult:false`, 101922-canary-green) + `normalizeListMedia`, on `window.franchiseFetch`. Live canary: `node scripts/anilist-discovery-canary.js` (ALL GREEN; run it after any data work — it stays OUT of the Playwright suite).
- **G2 cards+filter ✓** — `createDiscoveryCard(media)` (his 44 → gold "Reviewed by Blake" pin via `createCard(a,{pinned:true})`; else escaped outside card via `mediaToCardProps` + `scoreTier` chip + click→`openSecondaryModal`), `isNewlyReviewed` shimmer seen-state; `setFilterHasOutsideCards`/`matchesFilters` 3-way reviewed axis.
- **G3 (+3b/3c/3d/3e) Discover ✓** — `showDiscover`/`buildDiscoverSections`/`renderDiscoverInto`/`mountDiscoverCarousel` (native-scroll auto-drift = `discoverRails`), `freshSeed`/`seededShuffle`/`mulberry32`, `rankDiscoverResults`, `setDiscoverLens`, `discoverEmptyCard`/`railSkeleton`/`setRailMeta`; Den "Anime By Genre" = transform marquee (`denMarquees`); welcome-door polish + the Firefox focus fix + the `.wlp-glow` line-pulse.
- **G4 For You ✓** — `computeTasteProfile` (PURE taste engine: fav3/watch2/recent1 additive, `TASTE_GENRE_MAP` his-vocab→AniList, floor 4 / cap 3, "no gold-lead → no rail"), `buildForYou`/`buildForYouRail`/`fillForYouRailBands` (gold leads → `.foryou-rail-sep` gold-star seam → wider world), `editorsNote` byline, the relabeled lens, signed-out trending taster, the gift state, `pickFeaturedAnime` (personalized Den featured). Continue-rail clip fixed.
- **G5 real nav + home hole-fill ✓** — Direction C: `<nav.nav-places>`[Den/For You/Discover] | `.toolbar-seam` | `<div.nav-tools>`[View All/Random/Filter]; `setActivePlace` radio + `moveMarker` sliding gold marker (static `.place-btn.is-active::after` underbar floor; multi-row wrap fallback); `#den-btn` soft glide home (logo = hard reload). Home: `.den-folio` dated line, `#home-airing-block` (AIRING NOW), `#home-foryou-block` (For-You teaser), `lazyFillOnView` (IO-gated, API-free first paint). `wireCardSaveButtons` shared (catalog + `al:<id>` outside saves; column-reverse icon stack on badged cards).
- **G6 the veil ✓** — `body::after` Constellation veil over the city (`body::before` pushed `z-2`); `@property --veil-w0/w1/w2` wash alphas; per-surface density via `data-surface` on `<html>` (set in `setActivePlace`, the single funnel) crossfading ~450ms; door line-paths reused for continuity; `account.html`/`suggest.html` static `data-surface="foryou"`. Update Log RE-HOMED into the welcome door (`.welcome-changelog`); Top10+LatestDrop side-by-side (`.den-showcase`).
- **G7 final polish ✓** — veil lines radiate (STATIC glow halo, not an animation); showcase parity (purplish framed Latest-Drop panel matching the spotlight glass plate); centered airing chip; double-`showAll()` on `#all` fixed; card-a11y confirmed already-done; "N people requested this" DEFERRED to v1.9.0 (see NEXT.md — admin-read-only Firestore + free-text = a CF/privacy job).

### ⚠️ NEW traps this ship (in addition to §3 below — ALL still apply)
- **@property drives the veil crossfade** (`--veil-w0/w1/w2`, `<number>`, registered in style.css). Firefox-VERIFIED it interpolates (≥128). Fallback degrades to an instant swap (density still correct). Don't add an unrelated `:root` transition expecting it to survive reduced-motion.
- **The veil z-stack:** city `body::before` z-2 → veil `body::after` z-1 → content z 0+. `data-surface` is on `<html>` and is set ONLY in `setActivePlace` (every surface + View All's `null` route through it). Don't set it elsewhere.
- **⚠️ The home AIRING strip fetches LIVE AniList on EVERY home load** (it's near the top → its lazy IO fires post-paint). So the Playwright suite now hits AniList on most loads. Trap #6 still holds (never ASSERT live home content), and the two "no console errors" specs (`homepage-loads`, `modal-leak-check`) FILTER AniList-origin network noise via `msg.location().url`. This also makes the load-flakes worse (next bullet).
- **Full-suite-load FLAKES (got worse this ship):** several synchronous / Firebase / AniList-load tests fail ONCE under the full parallel suite and PASS isolated + on a clean re-run (`modal-leak-check`, the one-tab active-state test, `card-keyboard-a11y`, the account-density test). PROTOCOL: a single failure in a synchronous/DOM test → re-run it isolated before believing it; the suite is green on clean re-runs (verified 82/82 repeatedly).
- **The two motion carousels still coexist** (`discoverRails` native-scroll for Discover/For-You/home-strips vs `denMarquees` transform for "Anime By Genre"). The For-You + home rails reuse the Discover drift. `fillForYouRailBands` is SHARED by the For-You surface AND the home teaser (independent build tokens).

### 💭 Code's creative wants / banked ideas (mine — hand forward; full detail in NEXT.md "v1.8.4 — CODE'S BANKED IDEAS")
- **Protect the heart** stayed the spine all ship: gold pins lead every blended rail; the veil is darkest in the Den (his room). Keep it that way in any future surface.
- **The "Blake's Constellation" swing** (his gold cards drawn as connected stars) is DEFERRED twice now — it needs live card-rect measurement (per-frame, breaks the perf bar). The RIGHT version: a STATIC pre-baked SVG of line segments behind the Den's `.spotlight-stack` only (fixed decorative anchors, never live rects), reusing the `.wlp-glow` masked-pulse technique. A future polish gate.
- **The veil line PULSE** (an animated site-wide glow traveling the lines, "like the intro") was held back in G7 for the perf bar (an always-on animation is the one continuous compositor cost). If Blake's Profiler shows headroom + he wants it: the door's `.wlp-glow` technique extends, but needs a z-restructure (city z-3, veil z-2, a pulse element z-1) since there's no integer between the veil and content.
- **G6 tuning knobs Blake can still turn:** `--veil-w0: 0.72` if the Den reads flat; the held-out `--veil-stars` alpha-multiplier step (1.0/0.85/0.62) if wash-only "you are here" feels too subtle.
- **Open design calls flagged to Blake (he may react post-ship):** View All lights no nav place (could want it lit); the curator lens labels "The Blend / Blake's picks / The world" vs neutral; the dateline ribbon is on the new strips only (not the centered legacy sections).

### What v1.8.4 has BUILT so far (G1→G3e — all live in the working tree, all green)
> The blend works end-to-end: open the **Discover** button in the toolbar — live search pins Blake's 44 ("Reviewed by Blake" gold shelf) above the wider world, airing Top-10, airing-by-genre, community picks, all auto-drifting. The Den genre rails are an infinite marquee. The welcome door is polished. Cards are keyboard-navigable. **This is the foundation Gate 4 (For You) builds ON — reuse it, don't reinvent.**

**The discovery DATA layer (`franchise-fetch.js`, additive):** `searchMediaList(q, perPage, signal)` · `fetchTrendingList(perPage)` · `fetchAiringList(perPage, genre)` (3 flat queries `SEARCH_QUERY`/`TRENDING_QUERY`/`AIRING_QUERY`, all `type:ANIME isAdult:false`, 101922-canary-green) + `normalizeListMedia` (lean card shape). All on `window.franchiseFetch` + `module.exports`. Live canary: `node scripts/anilist-discovery-canary.js`.

**The caches + entry points (`script.js`):** `makeListCache(prefix, ttlMs, fetchFn)` factory (sibling of `makeDetailCache`) → `fetchTrendingCached` (rar:trending: **24h**) · `fetchAiringCached` (rar:airing: **12h**, keyed by genre or 'all') · `searchDiscover(q, signal)` (no cache; debounce+abort owns it). Exposed on **`window.rarDiscovery`** (also `createDiscoveryCard`, `mediaToCardProps`, `isNewlyReviewed`, `setFilterHasOutsideCards`, `rankDiscoverResults`, `seededShuffle`, `freshSeed`, `showDiscover`).

**The CARDS (the per-card blend):** `createDiscoveryCard(media)` — if the id matches one of his 44 (`findInCatalog`) → his REAL gold card + "Reviewed by Blake" pin (opens main modal); else an outside card (NOT-REVIEWED sticker, community-score chip tinted by `scoreTier()` high≥78/mid65-77/low/none="N/A", click → `openSecondaryModal(id)` the FREE path). `mediaToCardProps` ESCAPES every AniList string (titles are untrusted) + caps genres at top-2. The "newly-reviewed shimmer" seen-state is in two localStorage sets.

**The Discover SURFACE (`script.js`, all there):** `showDiscover`/`hideDiscover`, `buildDiscoverSections` (lazy — no AniList until opened), `renderDiscoverInto` (cards / branded skeleton `railSkeleton` / branded empty+error `discoverEmptyCard` — the v1.7.1 `.search-empty-card` vocabulary, 🔭, contextual CTA), `runDiscoverSearch`+`rankDiscoverResults` (pin his 44 + exact-title BOOST over AniList's messy SEARCH_MATCH), the **freshness seed** (`freshSeed` sessionStorage + `mulberry32` `seededShuffle` — stable mid-session, new each login), `setRailMeta` (dateline pill in `.discover-subrow`), the inline 3-way lens (`setDiscoverLens` — CSS-only `lens-all/reviewed/notyet`).

**⚠️ TWO carousel mechanisms coexist by design — DON'T confuse them:**
- **Discover rails = native-scroll auto-drift:** `mountDiscoverCarousel`/`discoverRails` Map/`discoverRailTick` (shared rAF + one IntersectionObserver, write-only `scrollLeft`, pause-on-interaction, reduced-motion static). **GATE 4 (For You) USES THIS ONE** (it's a browsing surface — SHIP-PROMPT says so).
- **Den "Anime By Genre" = infinite transform MARQUEE:** `mountDenMarquee`/`denMarquees` Map/`denMarqueeTick` (transform translate, one-direction, duplicated items, blur-in/out edges). Reverted from native-scroll in G3d (Blake course-correct) — transform composites for free = the perf win.

**The 3-way filter:** the panel segment (All/Reviewed/Unreviewed) + `setFilterHasOutsideCards(true)` (call it on any surface with outside cards — Gate 4 too) enables "Unreviewed". `matchesFilters` has a `reviewed` axis.

**The welcome door (G3e):** quote bubbles keep a **floor of 2** + **collision-avoidance** (`pickQuoteSlot` — constant-gap insight) + start immediately; a **line-pulse** (`.welcome-line-pulse`/`.wlp-glow` — a transform-scaled ring masked to the line paths, compositor-cheap) glows outward along the door's line-work; **the Enter-focus highlight is FIXED + Firefox-verified** (focus the dialog container `#welcome-splash` tabindex=-1, NOT the button — see trap #4 below).

**Cards are keyboard-navigable** (`role=button`, `tabindex=0`, Enter/Space keydown, `:focus-visible` ring, focus-within reveals fav/watch) — site-wide, from a spawned a11y task.

### Where v1.8.4 GOES (the gate map — follow SHIP-PROMPT's number, this is the shape)
**G1 data ✓ → G2 cards+filter ✓ → G3 Discover ✓ (+3b premium UI, +3c polish, +3d Den-marquee-revert+sparse, +3e door+Discover polish) ✓ → G4 For You (YOU ARE HERE) → G5 header nav (real Den/For You/Discover glide nav, replacing the provisional toolbar buttons) + home hole-fill (airing hero + For-You rail into the v1.8.3 "hole") → G6 BACKGROUND layer (the identity — see wants) → G7 brainstorm trio + polish → (G8 optional Quotes-admin page) → SWEEP (docs cascade + bump to 1.8.4 + audits + ONE Blake-authored commit + preview → prod on "ship it").**

### 💭 Code's creative wants for the rest of v1.8.4 (mine — hand forward; Blake invites design opinions, "go all out")
- **Protect the heart — even MORE on For You (G4).** Discover is "the wider world"; For You is "his picks NEAR your taste" — so his **pinned gold cards are the reward**, the wider-world cards the supporting cast. Don't let a For-You rail read as a generic recommendation feed. The brand-new-signed-in empty state (no saves yet) is a gift: warmly point them at Discover / the catalog to save things, in Blake's voice — don't waste it on a sad blank.
- **The "Blake reviewed this" gold pin is LANDED — protect it.** It's the per-card blend in miniature, the standout pitch. It's on Discover search results now; For You should use it too. If anything ever dims it, that's wrong.
- **G6 background IS the identity now (characters are DEAD).** This is the biggest creative swing left. **The line-pulse I built on the welcome door is the SEED VOCABULARY** — a transform-scaled glow ring masked to line-art, compositor-cheap. Extend THAT technique site-wide: deep-purple-black + etched line-work (Dir 1 "Lifting the veil"), the layer lifting per surface (Den darkest → Discover most open) so door→Den→For You→Discover feels like ONE room breathing. **Blake clarified in G1: the Call of the Night city backdrop STAYS** — the line/constellation layer is a translucent veil OVER it (dense in the Den, the city revealing as you go outward). Build it so the reveal is *felt*, not noticed. NOT scroll-linked (Gecko trap) — crossfade on the in-page glide.
- **Copy carries the brand — no provider names is a GIFT.** It forces Blake's voice into every empty state / kicker. The site already does this (隠れ家, "what the world's watching", the 🔭 "NOTHING OUT THERE"). Keep writing them like he talks.
- **Perf paranoia stays — Blake's headed Firefox Profiler is the arbiter (headless can't measure paint).** Two motion languages now run at once (Discover native-scroll drift + Den marquee). G6's background + the line-pulse are the next Gecko-sensitive things — stage each for the Profiler. I built a working **Firefox verification pattern** (standalone node script: `const {firefox}=require('@playwright/test')` + a `python -m http.server` — I used it to root-cause the focus bug live). USE IT for anything Firefox-specific or paint-sensitive; the Playwright suite is Chromium-only.
...
### 3. v1.8.4-build traps (NEW — these bit me; they'll bite you)
- **`window.animeData` is UNDEFINED.** `animeData.js` declares `const animeData = [...]` as a CLASSIC script → it's a global-LEXICAL binding, NOT a `window` property. In `page.evaluate` read the **bare global `animeData`** (a classic-realm eval can see it); `script.js` reads it as `window.animeData : animeData` fallback. A test using `window.animeData` SILENTLY SKIPS (I shipped that bug once — a skip is a phantom in disguise; chase 0-skip).
- **The secondary modal is `.secondary-layer` (a CLASS), not `#secondary-layer` (id).** `ensureSecondaryEl` sets `className='secondary-layer'`. A test/probe using the id selector returns "not visible" even when it IS open. (Cost me a confused debug loop.)
- **Firefox matches `:focus-visible` on a PROGRAMMATIC `.focus()`** (its heuristic treats script-focus as keyboard-focus). Chromium does NOT. So a `:focus-visible` ring can paint on auto-open in Firefox while looking fine in the Chromium test suite. This is why the welcome-Enter highlight survived two fixes. **Verify any focus/`:focus-visible` behavior in ACTUAL Firefox** (the standalone-firefox probe pattern, below). The fix that worked: focus the dialog *container*, not the button.
- **Sparse-centering must be OVERFLOW-based, not count-based.** A count threshold (`length<=4`) leaves a 5-7 card rail that FITS the width left-hugging. Toggle `is-sparse` on `el.scrollWidth <= el.clientWidth+1` (cards are fixed-width so it's accurate synchronously after render). Cost a whole wasted gate (G3c) before I root-caused it in G3d.
- **Two carousels coexist — see the YOU-ARE-HERE map.** `discoverRails` (native-scroll, Discover/For-You) vs `denMarquees` (transform marquee, Den). Pick the right one (For You = native-scroll).
- **Playwright flake:** the synchronous "one tab active" active-state test occasionally fails ONCE under full-suite load, passes isolated + on a clean re-run. If a purely-synchronous DOM test fails once, re-run isolated before believing it — it's a harness flake, not your code.
- **Live AniList stays OUT of the Playwright suite** (deterministic). Verify live data via `node scripts/anilist-discovery-canary.js` + the firefox/chromium standalone probes. The 30 discovery tests are all offline (assert exports/shape/wiring, not network).

### Firefox verification pattern (you WILL need it — the suite is Chromium-only)
```js
// _ffprobe.js (write to the project root so node resolves ./node_modules)
const { firefox } = require('@playwright/test');
const cp = require('child_process');
(async () => {
  const server = cp.spawn('python', ['-m','http.server','8796','--bind','127.0.0.1'], { stdio:'ignore' });
  await new Promise(r=>setTimeout(r,1500));
  const b = await firefox.launch(); const p = await b.newPage();
  await p.goto('http://127.0.0.1:8796/');
  // ... p.evaluate(() => el.matches(':focus-visible')) etc ...
  await b.close(); server.kill();
})().catch(e=>{ console.error('ERR', e.message); process.exit(1); });
// run: node _ffprobe.js   (then rm it — don't commit probes)
```
`npx playwright install firefox` is already done. This is how I root-caused the focus bug in Blake's actual browser.
