...
## Immediate next ship — v1.7.2 (multi-fetch data architecture + multi-hop revival)

**Closes the v1.6.10 architectural debt.** Build a batched parallel-fetch helper (`Promise.all`-based) for "given N AniList IDs, fetch their relations + streamingEpisodes in parallel and merge," then plumb in the two long-deferred items: **multi-hop franchise traversal** (Demon Slayer's Entertainment District / Swordsmith Village / Hashira Training / Infinity Castle arcs + OPM S3 via N+1 per-relation fetches) and **franchise-episode aggregation** (fix Re:Zero's wrong-season episodes). Now unblocked by v1.7.0's AniListId backfill. Full spec in Phase B-side below.
...
- **v1.7.x polish slots** (post-v1.7.3): ~~Romaji subtitle on cards~~ ✅ **shipped in v1.7.1** (romaji + native Japanese subtitle, both cards + modal). **AniList per-episode scores feasibility check** — confirm whether AniList's `Episode` GraphQL type exposes per-episode community scores. If so, layer into the episode list; if not, shelve. Deferred from v1.6.9 planning.
