...
<!-- author: Code | date: 2026-06-03 -->
## Immediate next ship — v1.7.4 (in-site secondary modal for franchise entries)

**Replace the open-externally jump with an in-site experience.** Today, clicking a non-catalog franchise row in the More Info panel opens the external entry page in a new tab. v1.7.4 opens a secondary modal *over* the primary one with a Back button, rendering detailed info per anime (extended description, episode list, characters, staff). Benefits from v1.7.3 — the `✓ REVIEWED` pills now resolve across every watched season, so in-catalog rows can deep-link to Blake's own review. Reuses the shared `franchise-fetch.js` data layer. Includes a "Not reviewed yet" treatment for ALSO-LIKED cards not in the catalog. **Estimated:** ~5-6 hours. Live state in `docs/HANDOFF.md`.
...
- **v1.7.4 — In-site secondary modal for franchise entries** (MINOR, pushed back from v1.7.3 by the watched-set slot-in). Replaces v1.6.8's "open AniList in new tab" with an in-site experience. Requires v1.7.2's data layer. Secondary modal opens over the primary modal with a Back button, renders detailed AniList info per anime (extended description, episode list, characters, staff). Includes watchlist + "Not Reviewed yet" treatment for ALSO LIKED cards not in catalog. Per-episode click-for-more-info if per-episode content source decided. Benefits from v1.7.3 — by the time this ships, "✓ Reviewed" pills work correctly across every watched season. Estimated ~5-6 hours.
