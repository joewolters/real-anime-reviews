...
## Immediate next ship — v1.7.0 (AniList backfill + MAL integration)

**Foundation ship for the v1.7.x arc.** Populates `AniListId` / `IdMal` / `AniListScore` / `AniListColor` columns on the existing ~44 reviews via Mode 1's pipeline. Excel + `animeData.js` both updated. Once shipped, every modal fetch can use the precise `Media(id:)` lookup instead of the popularity-sorted `Page(media:)` search — and v1.7.1's multi-fetch architecture (which closes the v1.6.10 architectural debt around multi-hop traversal + franchise-episode aggregation) becomes possible.
