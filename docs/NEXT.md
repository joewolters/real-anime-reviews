...
<!-- author: Code | date: 2026-06-03 -->
## Immediate next ship — v1.7.3 (Watched-set feature)

**Multi-season `✓ Reviewed` pills.** The in-catalog pill shipped in v1.7.2 matches only the primary `AniListId`, but Blake reviews ONE entry per franchise even when he's watched multiple seasons / OVAs / movies — so viewers see `✓ Reviewed` on S1 only, missing that S2 + the movie + the OVAs are also under his coverage. Scope:

1. **New `WatchedAniListIds` column** on `Anime_Master_Table.xlsx` — comma-separated list of every AniListId the single review actually covers; the sync script reads it into `animeData.js` as an array.
2. **Set-membership pill lookup** — `renderFranchiseEntry`'s catalog-pill check (v1.7.2) tests membership in the watched set instead of just the primary id.
3. **Admin multi-select UI** — `admin/new-anime.{html,js}` gets a franchise-tree multi-select (reuses v1.7.2's `traverseFranchise`) with checkboxes; default = all spine entries pre-checked, Blake unchecks stragglers.
4. **Mode 1 auto-fill** — fills the default watched-set since Blake "watches everything affiliated."
5. **One-time ~20-min backfill** across the 44 existing reviews.

**Estimated:** ~4-6 hours. Requires v1.7.2's `traverseFranchise` infrastructure (now shipped). Live state in `docs/HANDOFF.md`.
...
- **v1.7.3 — Watched-set feature** (MINOR). Today's "✓ Reviewed" pill only matches the primary `AniListId`; if Blake reviews a franchise that spans S1 + S2 + OVAs + movies, viewers should see the pill on EVERY watched entry, not just S1. Plan: new `WatchedAniListIds` column on `Anime_Master_Table.xlsx` (comma-separated list of every AniListId the single review actually covers), sync script reads it into `animeData.js` as an array, `renderFranchiseEntry`'s catalog-pill lookup checks set membership instead of just the primary id, admin form gets a multi-select UI showing the franchise tree (reuses `traverseFranchise` from v1.7.2) with checkboxes — default: all spine entries pre-checked, Blake unchecks stragglers. Mode 1 auto-fills the default since Blake "watches everything affiliated." One-time ~20-min backfill across 44 existing reviews. Slotted in 2026-06-03 during v1.7.2 gate 4 smoke. Estimated ~4-6 hours.
